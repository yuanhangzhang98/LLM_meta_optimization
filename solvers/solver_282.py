...
def _grad_single(vars, idx, sgn, hp):
    ...
    push_up = torch.relu(xs_star - xs) / xs_star  # in [0,1]
    ...
    # --- SINGLE CHANGE vs Design 274 ---
    # Need-triggered abs-width floor: only for high-xl clauses that are (i) in the below-γ window
    # and (ii) still need upward xs motion.
    band_sharp_abs_min = torch.as_tensor(hp.band_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    band_floor = band_sharp_abs_min * (1.0 + gate_xl * window * push_up)
