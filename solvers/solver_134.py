...
HYPER_SPACE = {
    ...
    # tie-only smoothing controls (TOP-2 only)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Hard top-2 and hard clause cost (exactly zero at satisfying Boolean assignments)
    top_val, top_idx = torch.topk(v_lit, 2, dim=-1)   # (M,2)
    unsat_top = 0.5 * (1.0 - top_val)                 # (M,2)
    c = unsat_top[:, 0]                               # (M,)

    # Tie detector based on top-2 margin
    ...
    # Soft split over TOP-2 only (used only when s ~ 1)
    ...
    # Best-literal routing support: {top1} normally; {top1,top2} only near ties
    ...
    # Gradient term: baseline-like, but tie-only smoothing over top-2
    G = c.unsqueeze(-1).expand_as(v_lit) + w_best * (u_other_3 - c.unsqueeze(-1))
    G = G * (xl * xs).unsqueeze(-1)

    # Rigidity term: STRICT hard argmax (no tie smoothing) to avoid dilution at large N
    R = onehot_top1 * c.unsqueeze(-1)
