...
def _grad_single(vars, idx, sgn, hp):
    ...
    # === smooth-knee Hill decay in log-xl + floor (SINGLE CHANGE vs 363: new floor coupling) ===
    ...
    sp = F.softplus(x)  # ~0 below knee; ~x above
    amp_decay = 1.0 / (1.0 + torch.pow(sp, amp_p))

    # CHANGED: floor depends on position within weak-band window (eta..gamma), not on c/gamma
    denom = (gamma - eta).clamp_min(1e-3)
    t = ((c - eta) / denom).clamp(0.0, 1.0)
    amp_floor_eff = (amp_floor * t).clamp(0.0, 0.95)
