...
def _grad_single(vars, idx, sgn, hp):
    ...
    near_gamma = (window * amp).clamp(0.0, 1.0)  # in [0,1]

    # ===== High-xl release gain with shifted breakpoint (as in Design 215),
    # SINGLE CHANGE: tail-safer near_eff for breakpoint shift.
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.2, device=v.device, dtype=v.dtype)  # fixed

    # Design 215 used: near_eff = near_gamma
    # Here: blend in weak_band mildly to avoid breakpoint motion outside release-active band.
    near_eff = (near_gamma * (0.5 + 0.5 * weak_band)).clamp(0.0, 1.0)
