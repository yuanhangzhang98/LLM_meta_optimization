...
HYPER_SPACE = {
    ...
    # NEW: clause-local leak that pulls xl toward 1 when the clause is satisfied (xs small)
    # Leak is gated by (1 - xs), so it turns off on violated clauses (xs ~ 1).
    "xl_leak":  dict(type="log_uniform", default=1e-1, low=1e-4, high=2.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW (single modification): clause-local xl leak when clause is satisfied (xs small)
    # Pulls xl back toward 1 without affecting violated clauses (xs ~ 1).
    dxl = dxl - hp.xl_leak * (1.0 - xs) * (xl - 1.0)
