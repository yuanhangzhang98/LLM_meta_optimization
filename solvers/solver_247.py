...
HYPER_SPACE = {
    # Backbone defaults (from the current best tuned region used in 244)
    ...
    # Below-gamma window geometry (one-sided)
    ...
    # Shifted-tanh warp parameters for sw(s)
    "warp_mu":  dict(type="uniform",     default=0.75,                  low=0.55, high=0.95),
    "warp_k":   dict(type="log_uniform", default=4.0,                   low=0.5,  high=20.0),

    # Thresholded bounded xl-gain on release amplitude
    "xl_gain": dict(type="uniform", default=0.6, low=0.0, high=2.0),

    # Ultra-late tail-only boost of hi toward 1 (from 244)
    "hi_tail_start": dict(type="uniform", default=0.97, low=0.90, high=0.995),
    "hi_tail_boost": dict(type="uniform", default=0.5,  low=0.0,  high=1.0),

    # Tail boost activates only when instance-level endgame indicator g is high.
    # (Default/range lowered because g is now a weighted MEAN over clauses.)
    "tail_gate_start": dict(type="uniform", default=0.25, low=0.05, high=0.80),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # shifted-tanh warp normalized to [0,1]
    warp_k = torch.as_tensor(hp.warp_k, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    warp_mu = torch.as_tensor(hp.warp_mu, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    eps = torch.as_tensor(1e-12, device=v.device, dtype=v.dtype)

    u = torch.tanh(warp_k * (s - warp_mu))
    u0 = torch.tanh(warp_k * (0.0 - warp_mu))
    u1 = torch.tanh(warp_k * (1.0 - warp_mu))
    sw = ((u - u0) / (u1 - u0 + eps)).clamp(0.0, 1.0)
    ...
    near_gamma = (window * amp).clamp(0.0, 1.0)

    # ===== Hi-ramp shift logic (as in 244/246) =====
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.2, device=v.device, dtype=v.dtype)
    xl_mid_eff = (mid0 + mid_shift * (0.5 - near_gamma)).clamp(0.05, 0.95)

    t_hi = (torch.relu(gate_xl - xl_mid_eff) / (1.0 - xl_mid_eff).clamp_min(1e-6)).clamp(0.0, 1.0)
    hi = t_hi * t_hi * t_hi * (t_hi * (t_hi * 6.0 - 15.0) + 10.0)  # C^2 in [0,1]

    # ===== Tail-only hi boost (from 244) + MODIFIED endgame gating =====
    t0 = torch.as_tensor(hp.hi_tail_start, device=v.device, dtype=v.dtype).clamp(0.0, 0.999)
    tb = torch.as_tensor(hp.hi_tail_boost, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)

    denom_tail = (1.0 - t0).clamp_min(1e-6)
    z = ((hi - t0) / denom_tail).clamp(0.0, 1.0)
    tail = z * z * z * (z * (z * 6.0 - 15.0) + 10.0)  # quintic smootherstep

    # CHANGE vs 246:
    # Use an INSTANCE-LEVEL endgame score g (gate_xl-weighted mean of local endgame scores)
    # so the tail boost triggers only when the *whole instance* is in endgame.
    g_local = (weak_band * near_gamma).clamp(0.0, 1.0)  # (M,)
    w = gate_xl.clamp(0.0, 1.0)
    g = (g_local * w).sum() / (w.sum() + 1e-12)          # scalar in [0,1]

    g0 = torch.as_tensor(hp.tail_gate_start, device=v.device, dtype=v.dtype).clamp(0.0, 0.999)
    dg = (1.0 - g0).clamp_min(1e-6)
    q = ((g - g0) / dg).clamp(0.0, 1.0)
    gate_tail = q * q * q * (q * (q * 6.0 - 15.0) + 10.0)  # scalar smootherstep

    eff_hi = (hi + tb * tail * gate_tail * (1.0 - hi)).clamp(0.0, 1.0)

    xl_gain = torch.as_tensor(hp.xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    release_gain = (1.0 + xl_gain * eff_hi)  # in [1, 1+xl_gain]
