...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 340: floor gate uses geometric-mean (sqrt soft-AND) instead of soft-OR ---
    floor_gate = torch.sqrt((weak_band * push_up).clamp(0.0, 1.0))
    floor_xl = (amp_floor * floor_gate).clamp(0.0, 1.0)
    amp_norm = (amp_pow + floor_xl * (1.0 - amp_pow)).clamp(0.0, 1.0)
