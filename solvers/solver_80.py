...
# ------------------------------------------------------------------ #
# 2.  HYPER_SPACE (same as Design-74)
# ------------------------------------------------------------------ #
HYPER_SPACE = {
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE MODIFICATION vs Design-74:
    # Replace logsumexp smooth-max with a kappa-tied sqrt smooth-max.
    # Behaves like hard max away from ties, but gives well-conditioned near-tie gradients.
    k = float(hp.kappa)
    smooth = max(0.25 * k, 1e-4)             # tie smoothing width tied to existing kappa
    eps_s = smooth * smooth
