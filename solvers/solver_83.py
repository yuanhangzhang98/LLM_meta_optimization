...
# 2.  HYPER_SPACE
# ------------------------------------------------------------------ #
# Match the strong cluster defaults (Design-81 / Design-74 tuned core)
HYPER_SPACE = {
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Conservative normalization from Design-81
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MODIFICATION vs Design-81:
    # Replace LSE smooth-max with sqrt smooth-max (Design-114 form), smoothing tied to kappa.
    k = float(hp.kappa)
    ...
    # Long-term memory stays driven by hard monitor (unchanged)
