...
HYPER_SPACE = {
    ...
    # g0/kappa now apply to a SPREAD-normalized gap in [0,1]
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # u in [0,1], hard clause cost is min(u)
    u = 0.5 * (1.0 - v_lit)  # (M,3)
    ...
    # SINGLE PRINCIPLED CHANGE (vs prior gates): normalize by clause spread (robustly clamped)
    spread = (u3 - u1).clamp(min=1e-6)
    gap_n = (gap / spread).clamp(0.0, 1.0)

    # w ~ 1 when gap_n is small (tie between best and 2nd-best relative to spread)
    ...
    # Long-term memory stays driven by hard monitor (planner constraint)
