...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Delay-only hi-ramp coupling (Design 219) with SINGLE CHANGE =====
    # Safeguard: do NOT include weak_band in near_eff (avoid double-gating delay).
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.2, device=v.device, dtype=v.dtype)

    near_eff = near_gamma.clamp(0.0, 1.0)  # <-- single change vs 219: removed * weak_band
