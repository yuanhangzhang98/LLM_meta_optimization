...
HYPER_SPACE = {
    ...
    # NEW: clause-local smooth clipping scale for memory updates (xl/xs) after dv-normalization
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: clause-local smooth clipping for memory updates only -----
    mem_norm = torch.maximum(dxl.abs(), dxs.abs()).clamp(min=1e-12)  # (M,)
    mem_damp = 1.0 / (1.0 + (mem_norm / hp.mem_clip))                # (M,) in (0,1]
    dxl = dxl * mem_damp
    dxs = dxs * mem_damp
