...
HYPER_SPACE = {
    ...
    # Absolute minimum weak-band width (Design 274 knob)
    "band_sharp_abs_min": dict(type="uniform", default=0.01,             low=0.001, high=0.08),

    # NEW (single change vs 274): bidirectional xl-conditioning for the abs-width floor
    # mult = clamp(1 + band_floor_xl*(2*gate_xl-1), [0.5, 2.0])
    # so easy clauses can narrow (quieter), hard clauses can widen (avoid starvation)
    "band_floor_xl": dict(type="uniform", default=1.0,                   low=0.0,  high=1.5),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 274: bidirectional xl-conditioning of the abs floor ---
    band_sharp_abs_min = torch.as_tensor(hp.band_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    band_floor_xl = torch.as_tensor(hp.band_floor_xl, device=v.device, dtype=v.dtype).clamp_min(0.0)
    mult = (1.0 + band_floor_xl * (2.0 * gate_xl - 1.0)).clamp(0.5, 2.0)
    band_floor = band_sharp_abs_min * mult
    ...
    mult2 = (1.0 + stag_margin_xl * (2.0 * gate_xl - 1.0)).clamp(0.5, 1.5)
    eff_margin = (stag_margin * mult2).clamp(0.0, 0.49)
