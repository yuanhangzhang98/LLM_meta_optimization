...
VARIABLES_SPEC = {
    ...
    # NEW (single principled change vs 347): smoothed xl-gate state for floor mixing
    "gxl": dict(init=lambda s: torch.zeros(s),
                shape=("B", "M"),
                bounds=(0.0, 1.0)),
}
...
HYPER_SPACE = {
    # Backbone (copied from 347 tuned region)
    ...
    # Soft-OR mix weight (xl-adaptive via smoothed gate)
    "floor_mix": dict(type="uniform", default=1.0, low=0.0, high=1.0),

    # NEW: smoothing rate for gxl tracking of gate_xl (smaller = smoother/less brittle)
    "tau_gxl": dict(type="log_uniform", default=5.0, low=0.1, high=50.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: smooth the xl gate for the floor-mix coupling
    tau_gxl = torch.as_tensor(hp.tau_gxl, device=v.device, dtype=v.dtype).clamp_min(0.0)
    dgxl = tau_gxl * (gate_xl - gxl)
    ...
    # (same as 347) soft-OR floor gate, but floor_mix is now gated by *smoothed* gxl
    floor_mix = torch.as_tensor(hp.floor_mix, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_mix_eff = (floor_mix * gxl).clamp(0.0, 1.0)
