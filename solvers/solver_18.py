...
HYPER_SPACE = {
    ...
    # New (single conceptual change): damped overshoot-enabled band release for xs
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Modified xs update: add damped, overshoot-enabled band release
    ...
    # xl gate in log-space (smooth; avoids triggering release on easy clauses)
    ...
    # Narrow band around gamma, allowing slight overshoot above gamma
    ...
    # Upward-only push on xs toward xs_star (bounded)
    xs_star = torch.as_tensor(hp.xs_star, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    push_up = torch.relu(xs_star - xs) / xs_star

    # Implicit damping above gamma to reduce oscillatory repeated crossings
    damp = torch.exp(-torch.relu(c - gamma) / band_sharp)

    release = hp.kappa * gate_xl * band * push_up * damp
