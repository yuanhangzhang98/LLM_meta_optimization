...
HYPER_SPACE = {
    ...
    # Clause-adaptive rigidity routing temperature (as in Design 5)
    # xs high (violated) -> smoother; xs low (satisfied) -> sharper
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== RIGIDITY TERM R (ONLY CHANGE): route over ALL 3 literals =====
    # Use per-literal unsatisfaction u (baseline-equivalent scale) for routing weights.
    u = 0.5 * (1.0 - v_lit)  # (M,3) in [0,1]
    tau = (hp.tau_floor + hp.tau_span * xs).clamp(min=1e-6)  # (M,)
    p = torch.softmax(-u / tau.unsqueeze(-1), dim=-1)        # (M,3)
