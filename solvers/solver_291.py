...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Strong tuned core defaults (from the best-performing cluster)
    ...
    # Tie-only smoothing knobs (dxs memory monitor only)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- dxs memory monitor: hard-min almost everywhere, soft only near genuine ties ---
    u = 0.5 * (1.0 - v_lit)  # (M,3) in [0,1]
    u_sorted, _ = torch.topk(u, 3, dim=-1, largest=False)  # ascending: u1<=u2<=u3
    u1 = u_sorted[:, 0]
    u2 = u_sorted[:, 1]
    u3 = u_sorted[:, 2]
    ...
    eps = 1e-6
    # Two bounded, well-conditioned tie indicators in [0,1]
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / spread.clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MOD vs prior gates: AND-like ambiguity via geometric mean
    gap_eff = torch.sqrt((g_mag * g_spread).clamp(min=0.0)).clamp(0.0, 1.0)

    w = torch.sigmoid((hp.g0 - gap_eff) / (hp.kappa + 1e-12)).clamp(0.0, 1.0)

    # c_soft restricted to [u1,u2] (top-2 expectation), avoids global smoothing
    u12 = u_sorted[:, 0:2]
