...
HYPER_SPACE = {
    # carried-over tuned defaults (Design 51)
    ...
    # NEW (single change vs Design 51): tapered violated-side reactivation bump strength
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (Design 51 + ONE change: tapered violated-side bump)
    ...
    # NEW: violated-side reactivation bump that is:
    #  - zero when pos=0 (no leak in satisfied region)
    #  - large just beyond boundary
    #  - decays for large pos (prevents Design-55-like constant boost)
    denom = (pos + tau)
    bump = (pos / denom.clamp(min=1e-12)) * torch.pow((tau / denom).clamp(0.0, 1.0), p)
