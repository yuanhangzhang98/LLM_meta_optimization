...
HYPER_SPACE = {
    # Baseline (best-known defaults from Design 78/86 backbone)
    ...
    # Bounded weak-band release (Design 78)
    ...
    # Below-gamma window geometry (fixed edges)
    ...
    # In-window amplitude shaping (Design 78)
    ...
    # NEW (single change): xl-conditioned effective denom for t-mapping inside the fixed window.
    # For high-xl clauses, we shrink denom (bounded) so t rises faster -> earlier/higher amp within window.
    "win_denom_xl": dict(type="uniform", default=0.0, low=0.0, high=2.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Weak-sat band: eta < c < gamma (fixed)
    ...
    # Below-gamma window: (gamma-stag_margin) < c < gamma (fixed edges)
    ...
    # In-window amplitude shaping (warp + quintic smootherstep)
    denom = (gamma - c0).clamp_min(1e-6)

    # SINGLE CHANGE: xl-conditioned effective denom for t (bounded)
    win_denom_xl = torch.as_tensor(hp.win_denom_xl, device=v.device, dtype=v.dtype).clamp_min(0.0)
    # denom_mult in [1, 2.5] so hard clauses can shrink denom by up to 2.5x
    denom_mult = (1.0 + win_denom_xl * gate_xl).clamp(1.0, 2.5)
    t = ((c - c0) / denom * denom_mult).clamp(0.0, 1.0)
    ...
    tw = t2 / (t2 + (1.0 - t2).pow(2) + eps)
    s = tw * tw * tw * (tw * (tw * 6.0 - 15.0) + 10.0)
