...
HYPER_SPACE = {
    # Backbone (seed from 347 tuned region)
    ...
    # NEW (single change vs 347): lower-floor on xl-gate ONLY for floor_mix coupling
    # gate_xl_floor = gate_xl + gate_floor*(1-gate_xl)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: debrittle floor-mix coupling by flooring the xl gate (no new state, instantaneous)
    gate_floor = torch.as_tensor(hp.gate_floor, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    gate_xl_floor = (gate_xl + gate_floor * (1.0 - gate_xl)).clamp(0.0, 1.0)
