...
HYPER_SPACE = {
    ...
    # --- SINGLE CHANGE vs Design 245 ---
    # Absolute minimum weak-band width to reduce starvation when band_sharp is tuned too small.
    "band_sharp_abs_min": dict(type="uniform", default=0.01,             low=0.001, high=0.08),

    # Upper weak-band edge warp (as in 245)
    "w_hi_shift": dict(type="uniform",     default=0.8813735870,         low=0.0,  high=1.5),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE: absolute minimum weak-band width floor ---
    band_sharp_abs_min = torch.as_tensor(hp.band_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    band_sharp_eff = torch.maximum(band_sharp, band_sharp_abs_min)

    w_lo = torch.sigmoid((c - eta) / band_sharp_eff)
    w_hi = torch.sigmoid((gamma - c) / band_sharp_eff)

    # Upper-edge warp (245)
    w_hi_shift = torch.as_tensor(hp.w_hi_shift, device=v.device, dtype=v.dtype).clamp(0.0, 5.0)
    w_hi_c = w_hi.clamp(1e-6, 1.0 - 1e-6)
    w_hi_soft = torch.sigmoid(torch.logit(w_hi_c) + w_hi_shift)
    weak_band = w_lo * w_hi_soft
