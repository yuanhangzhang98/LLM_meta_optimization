...
HYPER_SPACE = {
    ...
    # Bounded, gated release (as in 35/49/51/56)
    ...
    # In-window amplitude shaping (keep floor/pow from 56; add ONLY skew)
    ...
    # NEW: controls asymmetric placement of amplitude within the same window
    # win_skew > 0 delays the rise (more mass near the top edge); < 0 accelerates rise.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude: SINGLE CHANGE vs Design 56
    # smoothstep(t) -> asymmetric power-ratio ramp controlled by win_skew
    denom = (gamma - c0).clamp_min(1e-6)
    t = ((c - c0) / denom).clamp(0.0, 1.0)

    skew = torch.as_tensor(hp.win_skew, device=v.device, dtype=v.dtype)
    # Exponents >= 1 for flat-ish derivatives at the edges; skew shifts where ramp rises.
    a = 1.0 + torch.nn.functional.softplus(skew)      # rise exponent
    b = 1.0 + torch.nn.functional.softplus(-skew)     # fall exponent
