...
HYPER_SPACE = {
    # Backbone (seeded from 310/312 tuned region)
    ...
    "tail_mu":       dict(type="uniform",     default=0.12, low=0.0,  high=0.50),

    # Self-limiting amplitude at huge xl (310/312)
    "xl_norm": dict(type="log_uniform", default=3e4, low=1e3, high=1e6),

    # Ultra-late near-gamma window thickness (312)
    "near_margin": dict(type="uniform", default=0.03, low=0.0, high=0.20),

    # NEW (single change vs 312): near-gamma window shape/peakedness
    "near_pow": dict(type="log_uniform", default=2.0, low=0.25, high=8.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ultra-late near-gamma window (just below gamma)
    near_margin = torch.as_tensor(hp.near_margin, device=v.device, dtype=v.dtype).clamp(0.0, 0.99)
    c0 = (gamma - near_margin).clamp(0.0, 1.0)
    w_ng_lo = torch.sigmoid((c - c0) / band_sharp)
    w_ng_hi = torch.sigmoid((gamma - c) / band_sharp)
    near_gamma = (w_ng_lo * w_ng_hi).clamp(0.0, 1.0)

    # NEW: shape the near-gamma corridor without moving its location
    near_pow = torch.as_tensor(hp.near_pow, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    near_gamma = near_gamma.pow(near_pow)
    ...
    # tail-safety gate with xl-gated shift
    ...
    # self-limiting amplitude when xl is huge
    ...
    # (same as 312, but with shaped near_gamma)
    release = kappa * gate_xl * weak_band * near_gamma * push_up * gate_tail * amp_norm
