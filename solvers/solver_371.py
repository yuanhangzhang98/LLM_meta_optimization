...
def _grad_single(vars, idx, sgn, hp):
    ...
    # window-normalized coordinate
    denom = (gamma - eta).clamp_min(1e-3)
    t = ((c - eta) / denom).clamp(0.0, 1.0)

    # SINGLE CHANGE vs 367: concave (early-strong) floor coupling inside the safe window
    amp_floor_eff = (amp_floor * torch.sqrt(t)).clamp(0.0, 0.95)
