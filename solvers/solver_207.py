...
HYPER_SPACE = {
    ...
    # asymmetric xs knobs (Design 51/59/61)
    ...
    # violated-side boundary reactivation strength (Design 59/61)
    "kappa_on": dict(type="uniform",    default=0.04999601539141226,    low=0.0,  high=0.25),

    # NEW (single change vs Design 61): tune bump tail decay exp(-a_on*y)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (Design 61 + ONE small change)
    ...
    # violated-side reactivation bump: b(y)=(y/(1+y))^2 * exp(-a_on*y)
    # preserves b(0)=0 and zero boundary slope; a_on tunes support width.
    y = pos / tau
    a_on = torch.as_tensor(hp.a_on, device=c.device, dtype=c.dtype).clamp(min=1e-6)
    bump = torch.square(y / (1.0 + y)) * torch.exp(-a_on * y)
