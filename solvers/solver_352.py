...
HYPER_SPACE = {
    ...
    # SINGLE CHANGE vs 346: smooth power shaping for tail-gate activation (more tail-only, less brittle)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs 346: power-shaped tail gate (uses same xl_sharp; avoids 338-style brittleness) =====
    s_tail = (log_xl - log_thr_tail) / xl_sharp
    tail_pow = torch.as_tensor(hp.tail_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    gate_tail = torch.sigmoid(s_tail).pow(tail_pow)  # in (0,1), more tail-only as tail_pow increases

    # Broaden + progress-condition xl-temp activation (as in 346)
