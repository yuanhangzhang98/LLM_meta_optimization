...
HYPER_SPACE = {
    ...
    # NEW (single change vs Design 35): monotone xl-conditioning of below-γ window width
    # eff_margin = stag_margin * clamp(1 + stag_margin_xl*gate_xl, [1.0, 1.5])
    "stag_margin_xl": dict(type="uniform", default=0.3, low=0.0, high=1.5),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 35: monotone widening with gate_xl (no narrowing for easy clauses) ---
    stag_margin_xl = torch.as_tensor(hp.stag_margin_xl, device=v.device, dtype=v.dtype).clamp_min(0.0)
    mult = (1.0 + stag_margin_xl * gate_xl).clamp(1.0, 1.5)
