...
HYPER_SPACE = {
    ...
    # NEW: adaptive clause-local clip scale for dx only (post dv-normalization)
    "mem_clip":      dict(type="log_uniform", default=5.0,  low=0.1,  high=50.0),
    "mem_xl_tight":  dict(type="uniform",     default=0.5,  low=0.0,  high=2.0),
    "mem_c_boost":   dict(type="uniform",     default=2.0,  low=0.0,  high=8.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: clause-local smooth clipping for memory updates only, with adaptive clip scale -----
    # clip_eff increases with current violation (c) and decreases mildly with large xl.
    clip_eff = hp.mem_clip * (1.0 + hp.mem_c_boost * c) / (1.0 + hp.mem_xl_tight * torch.log1p(xl))
    clip_eff = clip_eff.clamp(min=1e-6)

    mem_norm = torch.maximum(dxl.abs(), dxs.abs()).clamp(min=1e-12)
    mem_damp = 1.0 / (1.0 + (mem_norm / clip_eff))
