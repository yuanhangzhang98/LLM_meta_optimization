...
HYPER_SPACE = {
    ...
    # NEW (single addition): xl-coupled hysteresis strength for xs turn-off threshold.
    # hys=0 recovers Design-10 xs switching (no hysteresis).
    "hys":     dict(type="uniform",     default=0.05, low=0.0,  high=0.25),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update: Design-10 asymmetric switch + NEW xl-dependent hysteresis on turn-off
    # Normalize xl to [0,1] using its known bounds to avoid scale sensitivity.
    xl_norm = torch.log1p(xl) / math.log(1.0 + 1e6)

    # Turn-on threshold stays at gamma.
    pos = torch.relu(c - hp.gamma)

    # Turn-off threshold decreases with xl (harder to turn off persistent/high-xl clauses).
    gamma_off = torch.clamp(hp.gamma - hp.hys * xl_norm, 0.0, 0.5)
    neg = torch.relu(gamma_off - c)

    dxs = hp.beta * ((1.0 - xs) * pos - (xs + hp.epsilon) * neg)
