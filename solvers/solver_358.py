...
HYPER_SPACE = {
    ...
    # SINGLE CHANGE vs 352: finite-support (smootherstep) tail gate with its own log-width
    "tail_width": dict(type="log_uniform", default=2.0, low=0.25, high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs 352: finite-support smootherstep tail gate =====
    # u=0 for log_xl<=log_thr_tail; u=1 for log_xl>=log_thr_tail+tail_width
    tail_width = torch.as_tensor(hp.tail_width, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    u = ((log_xl - log_thr_tail) / tail_width).clamp(0.0, 1.0)
    gate_tail = u * u * u * (u * (u * 6.0 - 15.0) + 10.0)  # quintic smootherstep

    # Broaden + progress-condition xl-temp activation
