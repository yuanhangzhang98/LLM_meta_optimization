...
HYPER_SPACE = {
    ...
    # Rigidity top-2 routing temperature with a near-satisfaction threshold:
    # tau(c) = tau_floor + tau_span * relu(c - c0)/(1 - c0)
    # => diffuse routing when c>c0; sharp routing (tau~floor) only when c<=c0.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== RIGIDITY TERM R (only change): top-2 routing with thresholded c-adaptive sharpness =====
    denom = (1.0 - hp.c0) + 1e-6
    t = torch.relu(c - hp.c0) / denom                  # in [0, ~1]
    tau = (hp.tau_floor + hp.tau_span * t).clamp(min=1e-6)  # (M,)
