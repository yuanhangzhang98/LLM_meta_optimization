...
HYPER_SPACE = {
    ...
    # SINGLE CHANGE vs 326: new bounded amp_norm family (smooth-knee Hill in log-xl + small c-coupled floor)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # === SINGLE CHANGE vs 326: smooth-knee Hill decay in log-xl with small c-coupled floor ===
    ...
    sp = F.softplus(x)  # smooth knee; ~0 below, ~x above
    amp_decay = 1.0 / (1.0 + torch.pow(sp, amp_p))

    # floor shrinks as c->0 (reduces late-stage over-release when dv is tiny)
    amp_floor_eff = (amp_floor * (c / gamma)).clamp(0.0, 0.95)
