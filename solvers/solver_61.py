...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Base (use Design-11 tuned defaults)
    ...
    # Tie-only smoothing knobs (unchanged ranges)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design 11: compact-support smoothstep tie gate
    # t=1 when gap<=g0-kappa, t=0 when gap>=g0; w is smooth interpolation on [0,1]
    k = hp.kappa
    t = ((hp.g0 - gap) / k).clamp(0.0, 1.0)
    w = (t * t * (3.0 - 2.0 * t)).clamp(0.0, 1.0)
