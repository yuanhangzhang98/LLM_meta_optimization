...
def _grad_single(vars, idx, sgn, hp):
    ...
    # baseline hard evaluation for v-dynamics
    top_val, top_idx = torch.topk(v_lit, 2, dim=-1)  # (M,2)
    unsat_top = 0.5 * (1.0 - top_val)                # (M,2)
    c_hard = unsat_top[:, 0]                         # (M,)

    # baseline G (hard/topk) and R (hard winner) for v-dynamics
    ...
    # --- ONLY change vs baseline: smooth clause cost for memory updates ---
