...
HYPER_SPACE = {
    # Backbone (seeded from 363 tuned region)
    ...
    # window-safe: eta = eta_frac * gamma, so eta < gamma always
    ...
    # In-window floor geometry (as in 377): convex power + small linear term
    "floor_pow":      dict(type="uniform", default=2.0,  low=1.0, high=6.0),
    "floor_lin_frac": dict(type="uniform", default=0.12, low=0.0, high=0.50),

    # NEW (single change driver): hinge-like early saturation for the floor shape
    # shape := clamp(shape0 / floor_tau, 0, 1); smaller => earlier full floor inside window
    "floor_tau": dict(type="uniform", default=0.80, low=0.30, high=1.00),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # tail-safety gate (anti-starvation sign)
    ...
    # amp_norm: smooth-knee Hill decay in log-xl + floor
    ...
    sp = F.softplus(x)  # smooth knee
    amp_decay = 1.0 / (1.0 + torch.pow(sp, amp_p))

    # window-normalized coordinate with guaranteed eta<gamma
    denom = (gamma - eta).clamp_min(1e-3)
    t = ((c - eta) / denom).clamp(0.0, 1.0)

    # ---- SINGLE CHANGE vs 377: hinge-like early-saturating floor coupling ----
    floor_pow = torch.as_tensor(hp.floor_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    floor_lin_frac = torch.as_tensor(hp.floor_lin_frac, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_tau = torch.as_tensor(hp.floor_tau, device=v.device, dtype=v.dtype).clamp(1e-3, 1.0)

    shape0 = (floor_lin_frac * t + (1.0 - floor_lin_frac) * torch.pow(t, floor_pow)).clamp(0.0, 1.0)
    shape = (shape0 / floor_tau).clamp(0.0, 1.0)  # earlier full floor when floor_tau < 1

    amp_floor_eff = (amp_floor * shape).clamp(0.0, 0.95)
    amp_norm = (amp_floor_eff + (1.0 - amp_floor_eff) * amp_decay).clamp(0.0, 1.0)
