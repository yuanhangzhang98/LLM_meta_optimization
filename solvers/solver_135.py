...
HYPER_SPACE = {
    ...
    # From Design 4: dv-anchored coupled normalization
    "kappa":     dict(type="log_uniform", default=0.1,   low=1e-3, high=1.0),

    # NEW (single change vs Design 4): cap normalization to avoid dv->0 overshoot/runaway
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Coupled, dv-anchored normalization (Design 4)
    dv_max = dv.abs().max()
    mem_max = torch.maximum(dxl.abs().max(), dxs.abs().max())
    denom = torch.maximum(dv_max, hp.kappa * mem_max).clamp(min=1e-6)
    scale = 1.0 / denom

    # NEW: anti-runaway cap for dv->0 regime
    scale = torch.minimum(scale, torch.as_tensor(hp.scale_cap, device=scale.device, dtype=scale.dtype))
