...
HYPER_SPACE = {
    ...
    # SINGLE NEW KNOB: floor for g_mag normalization to prevent gate miscalibration near u1,u2~0
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE MODIFICATION vs Design 80: add u_floor to improve gate calibration near u1,u2~0
    denom = (u1 + u2 + hp.u_floor).clamp(min=eps)
    g_mag = (gap / denom).clamp(0.0, 1.0)

    # Keep the spread feature as in Design 80
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # sqrt smooth-max for gap_eff, smoothing width tied to kappa (Design 80)
    k = float(hp.kappa)
    smooth = max(0.25 * k, 1e-4)
    eps_s = smooth * smooth

    a = g_mag
    b = g_spread
    s = torch.sqrt((a - b) * (a - b) + eps_s)
    gap_eff = (0.5 * (a + b + s) - 0.5 * math.sqrt(eps_s)).clamp(0.0, 1.0)
