...
HYPER_SPACE = {
    ...
    # NEW (vs baseline): severity-adaptive, per-variable tanh clipping for memory updates (applied after dv-normalization)
    "mem_clip":          dict(type="log_uniform", default=5.0,  low=0.1,  high=50.0),
    "mem_sev":           dict(type="uniform",     default=4.0,  low=0.0,  high=20.0),
    "mem_clip_xs_scale": dict(type="log_uniform", default=0.5,  low=0.05, high=5.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: severity-adaptive, per-variable tanh clipping (clause-local) -----
    # Stronger saturation for more violated clauses (larger c).
    clip_l = hp.mem_clip / (1.0 + hp.mem_sev * c)
    clip_s = (hp.mem_clip * hp.mem_clip_xs_scale) / (1.0 + hp.mem_sev * c)
    clip_l = clip_l.clamp(min=1e-6)
    clip_s = clip_s.clamp(min=1e-6)

    dxl = clip_l * torch.tanh(dxl / clip_l)
    dxs = clip_s * torch.tanh(dxs / clip_s)
