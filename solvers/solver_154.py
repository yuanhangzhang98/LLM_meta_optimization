...
HYPER_SPACE = {
    ...
    # Sparse violation-driven routing (single modification vs baseline)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== NEW: sparse assist to 2nd-best literal only when clearly violated =====
    # assist_gain = relu(c - c_thr) ensures near-zero activation for mildly violated clauses
    c_thr = torch.as_tensor(hp.c_thr, device=v.device, dtype=v.dtype)
    assist_gain = (c - c_thr).clamp_min(0.0)  # (M,)

    assist = torch.zeros_like(G)
    assist.scatter_(1, top_idx[:, 1:2], 1.0)
    assist *= (hp.eta * assist_gain).unsqueeze(-1) * (xl * xs).unsqueeze(-1)
