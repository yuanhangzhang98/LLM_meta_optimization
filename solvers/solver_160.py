...
HYPER_SPACE = {
    ...
    # Smooth violation-triggered Top-2 assist, but pressure-bounded via saturation
    ...
    # NEW: pressure saturation scale for assist: p_sat = p / (p + p0)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Pressure-bounded smooth assist on runner-up literal =====
    c_thr = torch.as_tensor(hp.c_thr, device=v.device, dtype=v.dtype)
    tau   = torch.as_tensor(hp.tau,   device=v.device, dtype=v.dtype)
    p0    = torch.as_tensor(hp.p0,    device=v.device, dtype=v.dtype)

    gate_c = torch.sigmoid((c - c_thr) / tau.clamp_min(1e-6))
    p_sat  = p / (p + p0.clamp_min(1e-12))  # in [0,1)

    assist = torch.zeros_like(G)
    assist.scatter_(1, top_idx[:, 1:2], 1.0)
    assist = assist * (hp.eta * c * gate_c * p_sat).unsqueeze(-1)
