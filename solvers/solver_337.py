...
HYPER_SPACE = {
    # Backbone (carry over tuned region used in 325/327/329/331/333/335)
    ...
    # SINGLE CHANGE (vs 333/335-style xl-temp): bound the softplus-of-log-xl excess
    # so xl-conditioning is smooth (not tail-only) but cannot over-soften (saturates).
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xl gate in log-space (bounded 0..1)
    ...
    # ===== SINGLE CHANGE: bounded xl-adaptive temperature =====
    # softplus(log-xl excess) gives smooth adaptation across xl;
    # tanh cap prevents unbounded over-softening at extreme xl.
    ...
    win_p_eff = win_p / (1.0 + xl_temp * xl_excess_bounded)
