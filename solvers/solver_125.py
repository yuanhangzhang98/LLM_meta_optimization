...
HYPER_SPACE = {
    ...
    # NEW: satisfaction-dependent xl relaxation (fast forgetting)
    "eta":     dict(type="log_uniform", default=5.0,  low=1e-1, high=50.0),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ONLY CHANGE vs baseline: add satisfaction-dependent fast forgetting of xl
    # For c > delta: identical to baseline
    # For c < delta: extra decay proportional to (delta-c) and (xl-1)
    satisfied_margin = (hp.delta - c).clamp(min=0.0)          # (M,)
    dxl = hp.alpha * (c - hp.delta) - hp.eta * satisfied_margin * (xl - 1.0)
