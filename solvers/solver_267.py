...
HYPER_SPACE = {
    ...
    # Near-gamma adaptive xl-threshold shift (Design 228)
    "thr_shift": dict(type="uniform", default=0.8, low=0.0, high=3.0),

    # --- SINGLE CHANGE vs Design 228 ---
    # Power-squash the near_eff signal to avoid premature/widespread threshold shifting.
    # near_eff = (near_gamma * weak_band)^near_pow, with near_pow>=1.
    "near_pow": dict(type="uniform", default=2.0, low=1.0, high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 228 =====
    # Power-squashed conditioning for threshold shift
    near_pow = torch.as_tensor(hp.near_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    near_eff = (near_gamma * weak_band).clamp(0.0, 1.0).pow(near_pow)

    thr_shift = torch.as_tensor(hp.thr_shift, device=v.device, dtype=v.dtype).clamp_min(0.0)
    log_thr_eff = (log_thr - thr_shift * near_eff).clamp(
        min=0.0,
