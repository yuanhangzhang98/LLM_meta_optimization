...
HYPER_SPACE = {
    ...
    # NEW (single change vs Design 5): xs-adaptive gate sharpness
    # kappa_eff = kappa / (1 + rho_xs * xs)
    "rho_xs":  dict(type="log_uniform", default=1.0,  low=1e-3, high=50.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Violation gate (Design-5) with xs-adaptive sharpness (NEW)
    kappa = torch.as_tensor(hp.kappa, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    rho_xs = torch.as_tensor(hp.rho_xs, device=v.device, dtype=v.dtype).clamp_min(0.0)
    kappa_eff = kappa / (1.0 + rho_xs * xs).clamp_min(1e-6)   # (M,)
