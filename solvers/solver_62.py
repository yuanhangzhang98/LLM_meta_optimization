...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Start from Design-11 tuned defaults for the base system
    ...
    # Gate threshold/sharpness for normalized tie-gap
    "g0":      dict(type="log_uniform", default=0.08, low=1e-3, high=1.0),
    "kappa":   dict(type="log_uniform", default=0.06, low=1e-3, high=5e-1),

    # NEW (single principled change): hybrid normalization weight for tie-gap denom
    # denom = spread + lam*(u1+u2); lam=0 -> spread-norm (Design 15), larger lam -> sum-norm flavor (Design 12)
    "lam":     dict(type="uniform", default=0.2, low=0.0, high=1.0),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design 11: hybrid normalized gap for tie gating
    spread = (u3 - u1).clamp(min=0.0)
    denom = (spread + hp.lam * (u1 + u2)).clamp(min=1e-6)
    gap_n = (gap / denom).clamp(0.0, 1.0)

    # w ~ 1 when gap_n is small (tie), w ~ 0 when clear winner
