...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- SINGLE CHANGE vs design 388: flip floor geometry to protect band-entry (anti-starvation) ----
    # was: amp_floor_eff = amp_floor * s^floor_pow (small near eta)
    # now: amp_floor_eff = amp_floor * (1-s)^floor_pow (largest near eta, vanishes near gamma)
    amp_floor_eff = (amp_floor * torch.pow((1.0 - s).clamp(0.0, 1.0), floor_pow)).clamp(0.0, 0.95)
