...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs 353: alternative (more selective) debrittling floor on xl-gate =====
    # 353 used: gate_xl_floor = gate_xl + gate_floor*(1-gate_xl)
    gate_floor = torch.as_tensor(hp.gate_floor, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    gate_xl_floor = (gate_xl + gate_floor * torch.pow(1.0 - gate_xl, 2.0)).clamp(0.0, 1.0)
