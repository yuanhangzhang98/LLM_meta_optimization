...
HYPER_SPACE = {
    ...
    # Design-15 style bounded weak-band release
    ...
    # NEW (single principled change vs Design 15): focus release on near-stagnant clauses
    # (clauses hovering close to gamma from below)
    "stag_margin": dict(type="uniform",     default=0.05,  low=0.0,  high=0.25),
    "stag_sharp":  dict(type="log_uniform", default=0.02,  low=1e-3, high=0.20),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Weak-sat band (same as Design 15): eta < c < gamma
    ...
    # NEW: stagnation proxy = clause is close to gamma from below
    ...
    near_gamma = torch.sigmoid((c - c0) / stag_sharp)        # ~1 only when c is near gamma
