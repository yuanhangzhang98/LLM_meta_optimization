...
HYPER_SPACE = {
    ...
    # Design-21: explicit off-rate multiplier for xs
    "eta":     dict(type="uniform",     default=0.2776329339570298,  low=0.0,  high=3.0),

    # NEW (single addition): minimum off-drive when clause is satisfied (anti-stickiness)
    # kappa=0 recovers Design-21.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update: Design-21 asymmetric switch + satisfied-gated minimum off-drive (anti-stickiness)
    pos = torch.relu(c - hp.gamma)            # violated beyond gamma
    neg = torch.relu(hp.gamma - c)            # satisfied beyond gamma

    sat = (neg > 0.0).to(v.dtype)             # mask: 1 iff satisfied
    off_drive = neg + hp.kappa * sat          # ensures nonzero decay once satisfied
