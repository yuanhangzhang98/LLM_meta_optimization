...
HYPER_SPACE = {
    ...
    # NEW (hybrid): smoothing only for clause monitor used in memory dynamics
    # smaller -> closer to hard min; larger -> smoother dxl/dxs near literal switches
    "tau_c":   dict(type="log_uniform", default=2e-2, low=1e-3, high=2e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Smooth clause monitor ONLY for memory dynamics ---
    # u in [0,1], clause cost approx min(u)
    u = 0.5 * (1.0 - v_lit)
    tau = hp.tau_c
    c_mem = (-tau * torch.logsumexp(-u / tau, dim=-1)).clamp(0.0, 1.0)  # (M,)
