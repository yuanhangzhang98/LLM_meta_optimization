...
HYPER_SPACE = {
    ...
    # Only new knob: smoothing temperature for memory clause cost c_mem.
    # Smaller -> closer to hard min; larger -> smoother memory updates.
    "tau_c":   dict(type="log_uniform", default=5e-3, low=1e-4, high=1e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- Baseline hard dv dynamics -----
    v_lit = v[idx] * sgn  # (M,3)

    # baseline hard winner/runner-up via topk on literal value
    ...
    # ----- Only change: smooth clause cost for memory (xl/xs) -----
    # per-literal unsatisfaction u in [0,1]
    u = 0.5 * (1.0 - v_lit)  # (M,3)

    tau_c = max(float(hp.tau_c), 1e-6)
    w = torch.softmax(-u / tau_c, dim=-1)             # (M,3)
