...
HYPER_SPACE = {
    ...
    # Debrittling floor on xl-gate ONLY for floor_mix coupling
    "gate_floor": dict(type="uniform", default=0.05, low=0.0, high=0.5),

    # NEW (single change vs 353): suppress added floor at moderate/high gate_xl
    # gate_xl_floor = gate_xl + gate_floor * (1-gate_xl)/(1 + gate_tau*gate_xl)
    "gate_tau": dict(type="uniform", default=0.5, low=0.0, high=1.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 353: suppressed floor mapping ---
    gate_floor = torch.as_tensor(hp.gate_floor, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    gate_tau   = torch.as_tensor(hp.gate_tau,   device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    gate_xl_floor = (gate_xl + gate_floor * (1.0 - gate_xl) / (1.0 + gate_tau * gate_xl)).clamp(0.0, 1.0)
