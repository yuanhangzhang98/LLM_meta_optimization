...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE MODIFICATION: smooth gating (freeze) of memory updates when satisfied/near-satisfied =====
    # Gate ~0 when c is below the respective threshold, ~1 when above.
    tau = 0.05
    gate_l = torch.sigmoid((c - hp.delta) / tau)
    gate_s = torch.sigmoid((c - hp.gamma) / tau)
    dxl = dxl * gate_l
    dxs = dxs * gate_s
