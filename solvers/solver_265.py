...
HYPER_SPACE = {
    # Backbone defaults (from best-known tuned region; Design 43 / 222)
    ...
    # Abs-min window floor (will be made state-conditional in dynamics)
    "stag_sharp_abs_min": dict(type="uniform", default=0.006, low=0.001, high=0.05),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    stag_sharp_eff_raw = stag_sharp / (1.0 + stag_sharp_gain * gate_xl)
    stag_sharp_eff_noabs = torch.maximum(stag_sharp_eff_raw, stag_sharp * sharp_min_ratio).clamp_min(1e-6)

    # ---------------- SINGLE CHANGE vs Design 230 ----------------
    # Make the abs-min width floor state-conditional (release-path only):
    # only when the clause is in the would-be near-gamma window, in weak-band,
    # and xl-gated (hard clause). This preserves anti-starvation without global widening.
    pre_window = torch.sigmoid((c - c0) / stag_sharp_eff_noabs)
    abs_gate = (pre_window * weak_band * gate_xl).clamp(0.0, 1.0)

    stag_sharp_abs_min = torch.as_tensor(hp.stag_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    abs_floor = (stag_sharp_abs_min * abs_gate).clamp_min(0.0)

    stag_sharp_eff = torch.maximum(stag_sharp_eff_noabs, abs_floor).clamp_min(1e-6)
    # ----------------------------------------------------------------
