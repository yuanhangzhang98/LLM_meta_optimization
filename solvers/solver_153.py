...
HYPER_SPACE = {
    ...
    # NEW (small hard-routing tweak): extra push to 2nd-best literal when clause is clearly violated
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # winner gets the runner-up unsat (baseline structure)
    G.scatter_(1, top_idx[:, 0:1], unsat_top[:, 1:2])
    G *= (xl * xs).unsqueeze(-1)

    # ===== NEW: hard assist to 2nd-best literal when clearly violated =====
    # violated_mask is hard-gated but vmap-safe
    c_thr = torch.as_tensor(hp.c_thr, device=v.device, dtype=v.dtype)
    violated = (c > c_thr).to(v.dtype)                  # (M,)

    assist = torch.zeros_like(G)
    assist.scatter_(1, top_idx[:, 1:2], 1.0)            # one-hot at 2nd-best literal
    assist *= (hp.eta * c * violated).unsqueeze(-1) * (xl * xs).unsqueeze(-1)
