...
HYPER_SPACE = {
    ...
    # Design 280: sharpen thr_shift conditioner
    "shift_pow": dict(type="uniform", default=2.5, low=1.0, high=6.0),

    # --- SINGLE CHANGE vs Design 280 ---
    # Soft-knee saturation for thr_shift activation (no dead-zone):
    # near_eff = a / (a + shift_tau^p), where a = near_raw^p.
    "shift_tau": dict(type="uniform", default=0.25, low=1e-3, high=0.90),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 280 =====
    # Soft-knee, saturating conditioner for thr_shift (no hard dead-zone).
    ...
    a = near_raw.pow(shift_pow)
    shift_tau = torch.as_tensor(hp.shift_tau, device=v.device, dtype=v.dtype).clamp_min(1e-6).clamp_max(0.999)
    b = shift_tau.pow(shift_pow)
    near_eff = (a / (a + b + eps)).clamp(0.0, 1.0)

    thr_shift = torch.as_tensor(hp.thr_shift, device=v.device, dtype=v.dtype).clamp_min(0.0)
    log_thr_eff = (log_thr - thr_shift * near_eff)
    log_thr_eff = log_thr_eff.clamp(min=0.0, max=torch.log(torch.as_tensor(1e6, device=v.device, dtype=v.dtype)))
