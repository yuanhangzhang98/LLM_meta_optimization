...
HYPER_SPACE = {
    ...
    # Over-gamma shoulder (interpreted as relative width in code below)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 386: make upper edge relative to remaining headroom (1-gamma) ---
    over_gamma = torch.as_tensor(hp.over_gamma, device=v.device, dtype=v.dtype).clamp_min(0.0)
    gamma_hi = (gamma + over_gamma * (1.0 - gamma)).clamp(0.0, 1.0)
