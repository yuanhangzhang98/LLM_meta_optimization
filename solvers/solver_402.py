...
HYPER_SPACE = {
    ...
    # Upper-edge sharpness (DECOUPLED)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Below-gamma window: (gamma-stag_margin) < c < gamma (lower edge)
    ...
    # --- SINGLE CHANGE vs design 393 ---
    # Shifted/scaled sigmoid so cutoff equals 1 at c=gamma_hi (instead of 0.5),
    # preserving shoulder mass while keeping smooth decay above gamma_hi.
    over_sharp = torch.as_tensor(hp.over_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    below_gamma = (2.0 * torch.sigmoid((gamma_hi - c) / over_sharp)).clamp(0.0, 1.0)
