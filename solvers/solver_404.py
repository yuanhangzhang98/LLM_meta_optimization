...
HYPER_SPACE = {
    # Backbone (seed from design 401 tuned region)
    ...
    # Tail-floor amplitude (kept)
    "tail_floor": dict(type="uniform", default=0.04, low=0.0, high=0.30),

    # NEW: make tail-floor engage only on a narrow manifold (rare + self-disabling)
    # s is the weak-band coordinate in [0,1] (near 1 means c≈gamma)
    "floor_s_thr":   dict(type="uniform",     default=0.90, low=0.50, high=0.98),
    "floor_s_sharp": dict(type="log_uniform", default=0.02, low=1e-3, high=0.20),
    # gate_tail_raw near-zero detector
    "floor_raw_thr":   dict(type="uniform",     default=0.12, low=0.00, high=0.50),
    "floor_raw_sharp": dict(type="log_uniform", default=0.02, low=1e-3, high=0.20),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # amp_decay: smooth-knee Hill decay in log-xl (computed BEFORE tail_floor; used to scale floor)
    ...
    sp = F.softplus(x)
    amp_decay = (1.0 / (1.0 + torch.pow(sp, amp_p))).clamp(0.0, 1.0)

    # ---- SINGLE PRINCIPLED MODIFICATION vs design 401 ----
    # Replace broad late/high-xl tail_floor with a sharp, rare, self-disabling floor:
    # engage only when (s close to 1) AND (gate_tail_raw near 0) and fade as xs rises (push_up),
    # and scale by amp_decay so it cannot dominate when amp_norm is held up by amp_floor.
    tail_floor = torch.as_tensor(hp.tail_floor, device=v.device, dtype=v.dtype).clamp(0.0, 0.95)
    floor_s_thr   = torch.as_tensor(hp.floor_s_thr, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_s_sharp = torch.as_tensor(hp.floor_s_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    floor_raw_thr   = torch.as_tensor(hp.floor_raw_thr, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_raw_sharp = torch.as_tensor(hp.floor_raw_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)

    near_gamma = torch.sigmoid((s - floor_s_thr) / floor_s_sharp).clamp(0.0, 1.0)
    raw_is_low = torch.sigmoid((floor_raw_thr - gate_tail_raw) / floor_raw_sharp).clamp(0.0, 1.0)
    floor_mask = (blend * near_gamma * raw_is_low * push_up * amp_decay).clamp(0.0, 1.0)
