...
HYPER_SPACE = {
    ...
    # Tie-only smoothing knobs (as in Design 11), but tie gate now uses normalized gap
    "tau_tie": dict(type="log_uniform", default=0.01712294456377018, low=1e-3, high=2e-1),
    # g0: threshold on normalized gap in [0,1]
    "g0":      dict(type="log_uniform", default=0.08, low=1e-3, high=1.0),
    # kappa: sharpness of transition on normalized gap
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design-11/7: normalize tie gap
    denom = (u1 + u2).clamp(min=1e-6)
    gap_n = (gap / denom).clamp(0.0, 1.0)

    # w ~ 1 when normalized gap is small (tie), w ~ 0 when clear winner
