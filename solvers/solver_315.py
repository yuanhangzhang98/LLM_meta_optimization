...
HYPER_SPACE = {
    # Baseline (kept from Design 25 tuned region)
    ...
    # Bounded, gated release (as in 25)
    ...
    # Below-gamma window geometry (two-sided, as in 25)
    ...
    # In-window amplitude shaping (as in 25)
    ...
    # SINGLE CHANGE vs Design 25: monotone edge-warp on the near-γ window
    # <1 softens/broadens window edges; >1 sharpens/selects more strongly.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 25: edge-warp the window (monotone, bounded) ---
    win_edge_pow = torch.as_tensor(hp.win_edge_pow, device=v.device, dtype=v.dtype).clamp_min(0.05)
    window_raw = (near_from_below * below_gamma).clamp(0.0, 1.0)
    window = window_raw.clamp_min(1e-12).pow(win_edge_pow)

    # In-window amplitude (quintic smootherstep, as in 25)
