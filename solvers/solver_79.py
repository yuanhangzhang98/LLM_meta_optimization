...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Design-11 tuned defaults (as used in 75/81/128 family)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Hard top-k structure for v-updates (baseline/81 unchanged) ---
    ...
    # Design-81 conservative tie detector: u2-normalized gap magnitude
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # Conservative smooth-max with temperature tied to kappa (as in 81)
    ...
    # SINGLE CHANGE vs Design-81:
    # bounded top-2 soft-argmin expectation (Design-75) => c_soft in [u1,u2]
