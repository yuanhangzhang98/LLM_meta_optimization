...
# 2.  HYPER_SPACE
# ------------------------------------------------------------------ #
# Match the strong tuned core used by Design-81/114/189
HYPER_SPACE = {
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Conservative normalization (Design-81)
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MODIFICATION vs Design-81:
    # Replace LSE smooth-max (with constant downward shift) by a softmax-weighted average,
    # which approaches hard max without a fixed bias away from ties.
