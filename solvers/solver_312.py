...
HYPER_SPACE = {
    # Backbone (seeded from 310 tuned region)
    ...
    # Self-limiting amplitude at huge xl (310)
    "xl_norm": dict(type="log_uniform", default=3e4, low=1e3, high=1e6),

    # NEW (single change vs 310): ultra-late, clause-local near-gamma window thickness
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: ultra-late near-gamma window (just below gamma)
    near_margin = torch.as_tensor(hp.near_margin, device=v.device, dtype=v.dtype).clamp(0.0, 0.99)
    w_ng_lo = torch.sigmoid((c - (gamma - near_margin).clamp(0.0, 1.0)) / band_sharp)
    ...
    # tail-safety gate with xl-gated shift (310)
    ...
    # SINGLE CHANGE vs 310: multiply by near_gamma to make release strictly late
