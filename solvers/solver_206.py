...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (Design 61 + ONE change: normalized Gaussian violated-side bump)
    ...
    # ONE MODIFICATION vs Design 61:
    # violated-side reactivation bump with (i) bump(0)=0, (ii) zero boundary slope, (iii) faster-than-exp decay,
    # and (iv) unit peak independent of tau: b(y)=e*y^2*exp(-y^2), peak at y=1 equals 1.
    y = pos / tau
    bump = torch.exp(torch.tensor(1.0, device=c.device, dtype=c.dtype)) * (y * y) * torch.exp(-(y * y))
