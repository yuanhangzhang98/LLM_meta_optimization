...
HYPER_SPACE = {
    # Baseline (best-known defaults from Design 51)
    ...
    # Bounded weak-band release (Design 51)
    ...
    # Below-gamma window geometry (Design 51)
    ...
    # In-window amplitude shaping (Design 83)
    ...
    # NEW (one change vs Design 40): shifted tanh warp steepness
    # sw = normalize(tanh(k*(s-mu))) with fixed mu=0.75; k tunes late-window ramp sharpness.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude shaping: smootherstep -> shifted tanh warp -> power
    ...
    # quintic smootherstep (C^2, flat edges)
    s = t2 * t2 * t2 * (t2 * (t2 * 6.0 - 15.0) + 10.0)

    # NEW: shifted tanh warp (monotone, bounded slope), normalized to hit [0,1]
    warp_k = torch.as_tensor(hp.warp_k, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    mu = torch.as_tensor(0.75, device=v.device, dtype=v.dtype)  # fixed center (single-change design)
    eps = torch.as_tensor(1e-12, device=v.device, dtype=v.dtype)

    u = torch.tanh(warp_k * (s - mu))
    u0 = torch.tanh(warp_k * (0.0 - mu))
    u1 = torch.tanh(warp_k * (1.0 - mu))
