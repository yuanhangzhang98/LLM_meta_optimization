...
HYPER_SPACE = {
    ...
    # NEW: temperature for smoothing G's per-literal responsibility (2-way softmin)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Clause cost (keep hard min as in baseline, but computed directly)
    c, best_pos = u.min(dim=-1)        # c: (M,), best_pos: (M,)

    # --- MOD: smooth the baseline G responsibility: for each literal p,
    #          use softmin over the OTHER TWO unsatisfactions.
    ...
    m_other = num_other / denom_other              # (M,3) ~ min(u of other two)

    # Gradient push term (smoothed responsibility, no topk/scatter)
    G = (xl * xs).unsqueeze(-1) * m_other          # (M,3)

    # Rigidity term: keep sharp/selective winner allocation (baseline-like)
