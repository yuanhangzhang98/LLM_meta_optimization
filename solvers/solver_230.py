...
HYPER_SPACE = {
    ...
    # --- SINGLE PRINCIPLED CHANGE vs Design 222 ---
    # Minimum ABSOLUTE window width to prevent starvation when stag_sharp is tuned too small.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Safe xl-adaptive sharpening with capped maximum sharpening (Design 222)
    ...
    stag_sharp_eff = torch.maximum(stag_sharp_eff_raw, stag_sharp * sharp_min_ratio)

    # --- SINGLE CHANGE: absolute minimum width floor ---
    stag_sharp_abs_min = torch.as_tensor(hp.stag_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    stag_sharp_eff = torch.maximum(stag_sharp_eff, stag_sharp_abs_min).clamp_min(1e-6)
