...
HYPER_SPACE = {
    ...
    # NEW: coupling strength for persistence-sensitive xl growth via xs
    # kappa=0 recovers baseline behavior.
    "kappa":   dict(type="uniform",     default=1.0,  low=0.0,  high=10.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: persistence-sensitive xl growth (only change)
    # Amplify (not gate) xl growth proportional to xs, using the same c.
    amp = 1.0 + hp.kappa * xs                  # in [1, 1+kappa]
    dxl = hp.alpha * (amp * (c - hp.delta))
