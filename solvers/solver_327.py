...
HYPER_SPACE = {
    ...
    # Window warp strength (logit-temperature)
    "win_edge_pow": dict(type="log_uniform", default=1.0, low=0.25, high=4.0),

    # Cap for bounded edge sensitivity
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design 325: smooth bounded "soft-cap" instead of hard clamp
    # (preserves bounded edge sensitivity but removes non-smooth thresholding)
    logit_w_warp = cap * torch.tanh((logit_w * win_p) / cap)
