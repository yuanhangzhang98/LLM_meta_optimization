...
HYPER_SPACE = {
    ...
    # Design-220 sharpening
    "stag_sharp_gain": dict(type="uniform", default=1.5,                low=0.0,  high=10.0),

    # --- SINGLE CHANGE vs Design 220 ---
    # Absolute safety floor on effective window sharpness to prevent over-localization/starvation
    # stag_sharp_eff = max(stag_sharp/(1+gain*gate_xl), stag_sharp_min)
    "stag_sharp_min": dict(type="log_uniform", default=0.006,            low=1e-3, high=0.06),
    ...
    # Bounded xl-dependent gain on release amplitude
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xl-adaptive sharpening with absolute safety floor
    ...
    stag_sharp_min = torch.as_tensor(hp.stag_sharp_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)

    stag_sharp_eff_raw = stag_sharp / (1.0 + stag_sharp_gain * gate_xl)
    stag_sharp_eff = torch.maximum(stag_sharp_eff_raw, stag_sharp_min).clamp_min(1e-6)
