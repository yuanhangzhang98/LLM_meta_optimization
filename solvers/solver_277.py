...
HYPER_SPACE = {
    # Backbone defaults (from best-known tuned region; as in Design 264/273)
    ...
    # Bounded weak-band release (Design 48 lineage)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Pressure (UNdamped here; change vs Design 273)
    ...
    # Near-sat-only damping gate (same construction as Design 273)
    ...
    gate_c = torch.sigmoid((c_damp - c) / c_damp_sharp)  # ~1 only when c is small (near satisfied)

    r = (pressure / p_cut).clamp_min(0.0)
    damp = 1.0 / (1.0 + gate_c * r.pow(nu))               # strictly positive

    # SINGLE CHANGE vs Design 273: damp the *total clause force* (G+R), not only pressure inside G
    dv_clause = (G + R) * sgn
    dv_clause = dv_clause * damp.unsqueeze(-1)
