...
HYPER_SPACE = {
    ...
    # Runner-up assist (Design 141 family): smooth c-gate + bounded pressure
    "eta":     dict(type="uniform",     default=1.6000793825659814,   low=0.0,  high=2.0),
    "c_thr":   dict(type="uniform",     default=0.5133524552515436,  low=0.50, high=0.95),
    "w":       dict(type="uniform",     default=0.014999859329706615, low=0.005, high=0.25),
    "p_sat":   dict(type="log_uniform", default=499.99674131425314, low=0.3,  high=1e3),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- NEW (single principled change): super-saturating assist pressure ---
    # Keep Design-141 smooth violation gate, but make assist vanish for very large xl*xs.
    ...
    # Super-saturating pressure: p_eff ~ p for small p, peaks near p~p_sat, then ~ p_sat^2/p -> 0.
    denom = (pressure + p_sat)
