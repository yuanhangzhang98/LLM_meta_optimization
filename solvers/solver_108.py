...
HYPER_SPACE = {
    ...
    # NEW (single principled change vs Design 19):
    # near-hard (but smooth) gate2 temperature tied to tau_floor via kappa
    "kappa":     dict(type="log_uniform", default=2.0,  low=0.1,  high=20.0),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== RIGIDITY TERM R (Design-19 tau(c), but smooth near-hard gate2) =====
    # tau(c)
    ...
    # Smooth gate2 with temperature coupled to tau_floor (near-hard when tau_floor is small)
    tau_floor = torch.as_tensor(hp.tau_floor, device=v.device, dtype=v.dtype).clamp(min=1e-6)
    kappa = torch.as_tensor(hp.kappa, device=v.device, dtype=v.dtype).clamp(min=1e-3)
    gate_temp = (kappa * tau_floor).clamp(min=1e-6)

    c0 = torch.as_tensor(hp.c0, device=v.device, dtype=v.dtype)
    gate2 = torch.sigmoid((c0 - c) / gate_temp)                 # ~1 for c<<c0, ~0 for c>>c0
