...
VARIABLES_SPEC = {
    ...
    "xs": dict(init=lambda s: torch.zeros(s),
               shape=("B", "M"),
               bounds=(0.0, 1.0)),
    # NEW: per-clause EMA of xs-decay strength (persistent-stall detector)
    "st": dict(init=lambda s: torch.zeros(s),
               shape=("B", "M"),
               bounds=(0.0, 1.0)),
}
...
HYPER_SPACE = {
    # Backbone (seeded tuned region from 398/400/403 family)
    ...
    # Tail-safety (conditional late/high-xl)
    ...
    # Anti-stall micro-release (keep design-405 knobs; trigger now uses EMA st)
    "anti_rho":       dict(type="uniform",     default=0.05,  low=0.0,  high=0.20),
    "gate_stuck":     dict(type="uniform",     default=0.15,  low=0.0,  high=0.50),
    "stuck_sharp":    dict(type="log_uniform", default=0.03,  low=1e-3, high=0.20),
    "anti_rhs_thr":   dict(type="uniform",     default=0.01,  low=0.0,  high=0.20),
    "anti_rhs_sharp": dict(type="log_uniform", default=0.02,  low=1e-3, high=0.20),
    "anti_cap":       dict(type="uniform",     default=0.02,  low=0.0,  high=0.08),

    # NEW: EMA rate for sustained-decay detector
    "anti_ema":       dict(type="log_uniform", default=0.05,  low=0.005, high=0.5),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # tail-safety gate (conditional late/high-xl)
    ...
    # ===== Anti-stall micro-release (single principled change: use EMA of decay strength) =====
    anti_rho       = torch.as_tensor(hp.anti_rho, device=v.device, dtype=v.dtype).clamp_min(0.0)
    gate_stuck     = torch.as_tensor(hp.gate_stuck, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    stuck_sharp    = torch.as_tensor(hp.stuck_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    anti_rhs_thr   = torch.as_tensor(hp.anti_rhs_thr, device=v.device, dtype=v.dtype).clamp_min(0.0)
    anti_rhs_sharp = torch.as_tensor(hp.anti_rhs_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    anti_cap       = torch.as_tensor(hp.anti_cap, device=v.device, dtype=v.dtype).clamp_min(0.0)

    # tail shut: gate_tail_raw below threshold
    tail_shut = torch.sigmoid((gate_stuck - gate_tail_raw) / stuck_sharp).clamp(0.0, 1.0)

    # decay strength from immediate rhs_base
    decay_strength = (-rhs_base).clamp_min(0.0)

    # NEW: EMA state st integrates decay_strength -> sustained-decay evidence
    anti_ema = torch.as_tensor(hp.anti_ema, device=v.device, dtype=v.dtype).clamp_min(0.0)
    dst = anti_ema * (decay_strength - st)

    # gate on sustained decay (st) rather than instantaneous decay_strength
    decay_gate = torch.sigmoid((st - anti_rhs_thr) / anti_rhs_sharp).clamp(0.0, 1.0)

    anti_raw = anti_rho * blend * weak_band * s * tail_shut * decay_gate * amp_norm
    anti = torch.minimum(anti_raw, anti_cap)

    dxs = hp.beta * (rhs_base + release + anti)
