...
HYPER_SPACE = {
    # Backbone (seed from 332 tuned region)
    ...
    "eta":        dict(type="uniform",     default=0.3321119651870279,  low=0.0,  high=0.49),
    ...
    # Self-limiting amplitude at huge xl + xl-conditional floor
    "xl_norm":   dict(type="log_uniform", default=15292.311320487264, low=1e3,  high=1e6),
    "amp_p":     dict(type="uniform",     default=2.0,             low=0.5,  high=4.0),
    "amp_floor": dict(type="log_uniform", default=0.01,            low=1e-6, high=0.1),

    # SINGLE CHANGE vs 332: delayed smooth ramp for floor activation in gate_xl
    # (protect head/mid by keeping floor ~off until gate_xl is sufficiently high)
    "gate_floor": dict(type="uniform", default=0.55, low=0.0, high=0.9),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # amp_norm: power-law decay + xl-conditional floor
    ...
    # --- SINGLE CHANGE vs 332: delayed smoothstep ramp in gate_xl ---
    gate_floor = torch.as_tensor(hp.gate_floor, device=v.device, dtype=v.dtype).clamp(0.0, 0.99)
    t = ((gate_xl - gate_floor) / (1.0 - gate_floor)).clamp(0.0, 1.0)
    # smoothstep: 3t^2 - 2t^3 (monotone, C1)
    t = (t * t * (3.0 - 2.0 * t)).clamp(0.0, 1.0)
    floor_xl = (amp_floor * t).clamp(0.0, 1.0)
