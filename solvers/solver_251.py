...
HYPER_SPACE = {
    ...
    # Ultra-late tail-only boost of hi toward 1 (kept)
    ...
    # Endgame gating threshold for tail boost
    "tail_gate_start": dict(type="uniform", default=0.75, low=0.30, high=0.95),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Tail-only hi boost (as in 246), but with ONE CHANGE: exponential slack-squeeze =====
    ...
    # Gate tail boost by endgame indicator g = weak_band * near_gamma
    g = (weak_band * near_gamma).clamp(0.0, 1.0)
    ...
    gate_tail = q * q * q * (q * (q * 6.0 - 15.0) + 10.0)  # quintic smootherstep

    # ONE SMALL CHANGE vs 246:
    # convert tb in [0,1] to lambda>=0 and squeeze slack by exp(-lambda * tail * gate_tail)
    lam = -torch.log((1.0 - tb).clamp_min(1e-3))
    eff_hi = 1.0 - (1.0 - hi) * torch.exp(-lam * tail * gate_tail)
    eff_hi = eff_hi.clamp(0.0, 1.0)
