...
HYPER_SPACE = {
    # Backbone defaults (from best-known tuned region around Design 215/236)
    ...
    # Hi-ramp sharpness in gate-space (as in 236)
    "hi_sharp": dict(type="log_uniform", default=0.04, low=5e-3, high=0.2),

    # NEW (single change vs 236): rational warp power for hi in [0,1]
    # p>1 boosts tail (hi near 1) while minimally affecting early ramp.
    "hi_warp": dict(type="log_uniform", default=2.0, low=0.5, high=8.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Hi-ramp (Design 236) + SINGLE CHANGE: rational warp on hi =====
    ...
    # Monotone bounded warp: boosts tail without changing endpoints
    p = torch.as_tensor(hp.hi_warp, device=v.device, dtype=v.dtype).clamp_min(1e-3)
    hi = hi / (hi + (1.0 - hi).pow(p) + 1e-12)
    hi = hi.clamp(0.0, 1.0)
