...
HYPER_SPACE = {
    ...
    # Soft-OR mix weight (kept), now used in an xl-adaptive way
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 345: make push_up mixing weight smoothly xl-adaptive ---
    floor_mix = torch.as_tensor(hp.floor_mix, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_mix_eff = (floor_mix * gate_xl).clamp(0.0, 1.0)
