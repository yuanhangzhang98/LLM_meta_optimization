...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Baseline params (defaults set to a previously good region from Design 65)
    ...
    # Tie-aware xs monitor knobs (Design 65)
    "tau_tie": dict(type="log_uniform", default=0.01712294456377018, low=1e-3, high=2e-1),
    "g0":      dict(type="log_uniform", default=0.10,                low=1e-3, high=1.0),
    "kappa":   dict(type="log_uniform", default=0.06,                low=1e-3, high=5e-1),
    "g_floor": dict(type="log_uniform", default=0.02,                low=1e-3, high=2e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE MODIFICATION vs baseline: tie-aware smooth monitor for dxs only ---
    # unsat per literal
    u = 0.5 * (1.0 - v_lit)  # (M,3) in [0,1]
    u12, _ = torch.topk(u, 2, dim=-1, largest=False)  # (M,2) smallest two (best literals)
    u1 = u12[:, 0]
    u2 = u12[:, 1]

    gap = (u2 - u1).clamp(min=0.0)
    denom = (u1 + u2 + hp.g_floor).clamp(min=1e-6)
