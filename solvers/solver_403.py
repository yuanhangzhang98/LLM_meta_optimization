...
HYPER_SPACE = {
    # Backbone (seeded tuned region from 398)
    ...
    # Tail-safety (conditional, from 398)
    ...
    # NEW (single principled change vs 398): conditional anti-stall micro-release
    # activates only when late/high-xl, in-band near top, tail-protected, and xs is low;
    # saturates and turns off once xs rises.
    "anti_rho":     dict(type="uniform",     default=0.12,  low=0.0,  high=0.50),
    "xs_stuck":     dict(type="uniform",     default=0.22,  low=0.02, high=0.60),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== NEW: conditional anti-stall micro-release (no global tail floor) =====
    anti_rho    = torch.as_tensor(hp.anti_rho, device=v.device, dtype=v.dtype).clamp_min(0.0)
    xs_stuck    = torch.as_tensor(hp.xs_stuck, device=v.device, dtype=v.dtype).clamp(1e-6, 1.0)
    ...
    stuck_xs = (torch.relu(xs_stuck - xs) / xs_stuck).clamp(0.0, 1.0)
    tail_shut = torch.sigmoid((gate_stuck - gate_tail_raw) / stuck_sharp).clamp(0.0, 1.0)

    # near-top-of-band emphasis via s, and late/high-xl via blend
    anti = anti_rho * blend * weak_band * s * tail_shut * stuck_xs * amp_norm
