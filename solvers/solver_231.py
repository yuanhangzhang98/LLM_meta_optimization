...
HYPER_SPACE = {
    # Baseline (best-known defaults from Design 51 / carried into Design 38)
    ...
    # In-window amplitude shaping (Design 38)
    ...
    # NEW (one change vs Design 38): tunable exponent in the *post-power* rational warp
    # Design 38 used sw = sp / (sp + (1-sp)^2). Here 2 -> warp_post_p.
    "warp_post_p": dict(type="uniform",     default=2.0,                  low=1.0,  high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude shaping (Design 38 ordering): smootherstep -> power -> warp
    ...
    # NEW: tunable rational warp exponent after power
    warp_post_p = torch.as_tensor(hp.warp_post_p, device=v.device, dtype=v.dtype).clamp_min(1.0)
    eps = torch.as_tensor(1e-12, device=v.device, dtype=v.dtype)
    sw = sp / (sp + (1.0 - sp).clamp(0.0, 1.0).pow(warp_post_p) + eps)

    win_floor = torch.as_tensor(hp.win_floor, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    amp = win_floor + (1.0 - win_floor) * sw
