...
HYPER_SPACE = {
    ...
    # NEW: bounded Top-2 assist (single violation gate + xs-bounded amplitude)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== NEW: bounded assist to runner-up literal (decoupled from xl) =====
    ...
    gate_c = torch.sigmoid((c - c_thr) / tau)  # monotone in c, in (0,1)

    assist = torch.zeros_like(G)
    assist.scatter_(1, top_idx[:, 1:2], 1.0)
    # amplitude bounded by eta * c * xs (since gate_c<=1, xs<=1)
    assist *= (hp.eta * c * gate_c * xs).unsqueeze(-1)
