...
HYPER_SPACE = {
    ...
    # Ultra-late tail-only boost of hi toward 1
    ...
    # Tail gate threshold (now used clause-locally; same knob)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Tail-only hi boost (from 244) + CLAUSE-LOCAL endgame gating (single change vs 247) =====
    ...
    # OLD (247): gate_tail depended on instance-level mean g
    # NEW: clause-local gate from local endgame score g_local = weak_band*near_gamma
    ...
    q = ((g_local - g0) / dg).clamp(0.0, 1.0)
    gate_tail = q * q * q * (q * (q * 6.0 - 15.0) + 10.0)  # (M,) smootherstep
    ...
    release_gain = (1.0 + xl_gain * eff_hi)  # (M,)
