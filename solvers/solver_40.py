...
HYPER_SPACE = {
    ...
    # In-window amplitude shaping (Design 83) + ONE CHANGE: tunable rational warp power
    ...
    # NEW: warp exponent p in sw = s / (s + (1-s)^p); p=2 recovers Design 83
    "warp_p":   dict(type="uniform",     default=2.0,                    low=1.0,  high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude shaping: smootherstep -> (tunable) warp -> power
    ...
    # NEW: tunable monotone rational warp
    warp_p = torch.as_tensor(hp.warp_p, device=v.device, dtype=v.dtype).clamp_min(1.0)
    eps = torch.as_tensor(1e-12, device=v.device, dtype=v.dtype)
    sw = s / (s + (1.0 - s).clamp_min(0.0).pow(warp_p) + eps)
