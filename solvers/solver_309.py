...
HYPER_SPACE = {
    # baseline / 208-family defaults (from 262/268/308)
    ...
    # tie-aware near-min correction (ONLY in dxs via c_mem)
    "tau_tie":  dict(type="log_uniform", default=0.02,                  low=1e-4, high=2e-1),
    "a_tie":    dict(type="uniform",     default=0.25,                  low=0.0,  high=0.6),

    # NEW (single change vs 308): decay shape exponent for tie correction
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- tie-aware monitor for xs switching (used ONLY in dxs) ---
    u = 0.5 * (1.0 - v_lit)  # (M,3) per-literal unsat in [0,1]
    u12, _ = torch.topk(u, 2, dim=-1, largest=False)  # (M,2): u1<=u2
    u1 = u12[:, 0]
    u2 = u12[:, 1]
    gap = (u2 - u1).clamp(min=0.0)

    tau = torch.as_tensor(hp.tau_tie, device=v.device, dtype=v.dtype).clamp(min=1e-8)
    a   = torch.as_tensor(hp.a_tie,   device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    p   = torch.as_tensor(hp.p_tie,   device=v.device, dtype=v.dtype).clamp(1.0, 10.0)

    # SINGLE CHANGE vs 308: power-rational decay with exponent p_tie
    gap_p = torch.pow(gap.clamp(min=0.0) + eps, p)
    tau_p = torch.pow(tau, p)
    phi = a * (tau_p / (gap_p + tau_p + eps))  # in [0,a]

    c_mem = (u1 + gap * phi).clamp(0.0, 1.0)   # stays in [u1,u2] when a<=1
