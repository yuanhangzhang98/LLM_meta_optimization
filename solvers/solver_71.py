...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Start from Design-11 tuned defaults (as in 18/26)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Memory monitor for dxs: same tie gate, improved tie smoother ---
    # u in [0,1]
    u = 0.5 * (1.0 - v_lit)  # (M,3)

    # u1<=u2 are used for tie detection and bounding
    ...
    # PRINCIPLED SINGLE CHANGE:
    # Use entropy-corrected softmin over ALL 3 literals (log-mean-exp), then clamp to [u1,u2].
    # This avoids c_mem < u1 (which can under-drive xs near ties), while keeping smoothness.
    tau = hp.tau_tie
    lme3 = -tau * (torch.logsumexp(-u / tau, dim=-1) - math.log(3.0))  # >= min(u)
