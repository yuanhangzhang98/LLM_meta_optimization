...
HYPER_SPACE = {
    # baseline / 208-family defaults (from 268)
    ...
    # SINGLE CHANGE vs 268: use bounded Boltzmann top-2 average temperature (only in dxs)
    "tau_tie":   dict(type="log_uniform", default=0.02,                low=1e-4, high=1e-1),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # C∞ compact-support bump tie window (as in 268)
    ...
    # SINGLE CHANGE vs 268: bounded Boltzmann-weighted top-2 average in [u1,u2]
    tau = torch.as_tensor(hp.tau_tie, device=v.device, dtype=v.dtype).clamp(min=1e-6)
    # stabilize by shifting by u1
    wts = torch.exp(-(u12 - u1.unsqueeze(-1)) / tau).clamp(min=eps)
    c_boltz2 = (wts * u12).sum(dim=-1) / wts.sum(dim=-1).clamp(min=eps)
    c_boltz2 = c_boltz2.clamp(0.0, 1.0)

    c_mem = ((1.0 - w) * u1 + w * c_boltz2).clamp(0.0, 1.0)
