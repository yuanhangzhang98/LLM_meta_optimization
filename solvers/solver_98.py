...
HYPER_SPACE = {
    ...
    # NEW: relative allowance for memory-update magnitude in the shared normalization.
    # After scaling, max(|dxl|,|dxs|) is kept 
    # approximately <= mem_rel * max(|dv|) (before dv is normalized).
    "mem_rel":  dict(type="log_uniform", default=10.0, low=1.0,  high=100.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: joint normalization (single change vs baseline) -----
    max_v = dv.abs().max()
    max_mem = torch.maximum(dxl.abs().max(), dxs.abs().max())
    denom = torch.maximum(max_v, max_mem / hp.mem_rel)
    scale = 1.0 / denom.clamp(min=1e-6)
