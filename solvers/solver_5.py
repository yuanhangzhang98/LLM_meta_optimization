...
HYPER_SPACE = {
    ...
    # Anti-stall: prevent xs->0 (rigidity lock) for historically hard clauses (large xl)
    # when they are only weakly satisfied (0 < c < gamma).
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Anti-stall modification (ONLY xs dynamics):
    # add a positive bump when xl is large and clause is weakly satisfied (0<c<gamma),
    # to keep xs from collapsing and over-amplifying rigidity late in the run.
    ...
    bump = hp.kappa * gate_xl * c * weak_sat                  # vanishes at c=0 and at c>=gamma
