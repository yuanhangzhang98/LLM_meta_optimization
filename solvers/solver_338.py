...
HYPER_SPACE = {
    ...
    # xl-adaptive temperature magnitude (as in 329)
    "xl_temp": dict(type="uniform", default=1.0, low=0.0, high=5.0),

    # Tail-only xl-temp (as in 335)
    "xl_tail_mult": dict(type="log_uniform", default=10.0, low=1.0, high=100.0),

    # SINGLE CHANGE vs 335: independent sharpness for tail-gate turn-on
    "xl_tail_sharp": dict(type="log_uniform", default=0.7, low=1e-1, high=5.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xl gate in log-space (used for release as in 329/335)
    ...
    xl_tail_sharp = torch.as_tensor(hp.xl_tail_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)

    log_thr_tail = log_thr + torch.log(xl_tail_mult)

    # SINGLE CHANGE vs 335: tail gate uses its own sharpness
    gate_tail = torch.sigmoid((log_xl - log_thr_tail) / xl_tail_sharp)

    win_p_eff = win_p / (1.0 + xl_temp * gate_tail)  # soften only for extreme-tail xl
