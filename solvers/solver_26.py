...
HYPER_SPACE = {
    ...
    # Bounded, gated release (as in 35/49/51/56/60)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude: SINGLE CHANGE vs Design 60
    # quintic smootherstep(t) -> raised-cosine (Hann) ramp: s(t)=0.5*(1-cos(pi*t))
    denom = (gamma - c0).clamp_min(1e-6)
    t = ((c - c0) / denom).clamp(0.0, 1.0)

    pi = torch.as_tensor(math.pi, device=v.device, dtype=v.dtype)
    s = 0.5 * (1.0 - torch.cos(pi * t))
