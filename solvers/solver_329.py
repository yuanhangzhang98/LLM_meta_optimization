...
HYPER_SPACE = {
    # Backbone (carry over the tuned region used in 325/327)
    ...
    # SINGLE NEW DOF (the only change vs 327): xl-adaptive temperature for the window edges
    # Larger -> more softening for high-xl clauses (reduces brittle switching in the tail).
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs 327: make the edge warp xl-adaptive (soften for high-xl clauses)
    xl_temp = torch.as_tensor(hp.xl_temp, device=v.device, dtype=v.dtype).clamp_min(0.0)
    win_p_eff = win_p / (1.0 + xl_temp * gate_xl)  # (M,)
