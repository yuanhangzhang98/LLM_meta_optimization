...
HYPER_SPACE = {
    ...
    # Bounded weak-band release (Design 106/114)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 46: near_gamma-coupled onset for high-xl ramp =====
    # Effective breakpoint shifts up when not near_gamma (delays hi-gain),
    # and returns to 0.5 when near_gamma≈1 (preserves tail behavior of 46).
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.25, device=v.device, dtype=v.dtype)  # fixed, no new knob
    xl_mid_eff = (mid0 + mid_shift * (1.0 - near_gamma)).clamp(0.05, 0.95)
