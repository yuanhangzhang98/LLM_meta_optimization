...
HYPER_SPACE = {
    ...
    # Runner-up assist family (Design 141/142)
    ...
    # NEW (single conceptual change vs Design 142): tunable tail suppression in super-saturation
    # k_tail=1 reproduces Design-142 p_eff ~ p_sat^2 * p / (p+p_sat)^2.
    "k_tail":  dict(type="uniform",     default=1.0,   low=0.25, high=3.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Assist: smooth violation gate + super-saturating (peak-then-decay) effective pressure
    ...
    k_tail = torch.as_tensor(hp.k_tail, device=v.device, dtype=v.dtype).clamp_min(0.05)

    z = ((c - c_thr) / w).clamp(-20.0, 20.0)
    gate = torch.sigmoid(z)  # (M,)

    denom = (pressure + p_sat).clamp_min(1e-6)
    # Generalized Design-142: p_eff = (p_sat * p / (p+p_sat)) * (p_sat/(p+p_sat))**k_tail
    p_eff = (p_sat * pressure / denom) * (p_sat / denom).pow(k_tail)
