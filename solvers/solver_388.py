...
HYPER_SPACE = {
    # Backbone (seeded from the 363/373 tuned region used by 375/385/379)
    ...
    # In-window floor geometry (kept as in 375)
    "floor_pow": dict(type="uniform",     default=2.0,             low=1.0,  high=6.0),

    # NEW (single change driver): smooth-max temperature for late-only floor engagement
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # floor shape in weak band (kept as in 375)
    ...
    # ---- SINGLE CHANGE vs 375: late-acting floor via smooth-max lower bound ----
    # Engages only when amp_decay has fallen below amp_floor_eff (tail / large xl), avoiding early clamp.
