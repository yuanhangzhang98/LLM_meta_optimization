...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE MODIFICATION (vs Design 15): robust combined normalization
    # - magnitude-normalized: gap/(u1+u2)
    # - spread-normalized:    gap/(u3-u1)
    # Use max so we only call it a tie if BOTH notions say it's a tie.
    eps = 1e-6
    g_mag = (gap / (u1 + u2).clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)
    gap_eff = torch.maximum(g_mag, g_spread)  # in [0,1]
