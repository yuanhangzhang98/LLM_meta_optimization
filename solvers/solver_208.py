...
HYPER_SPACE = {
    # carried-over tuned defaults (Design 61)
    ...
    # asymmetric xs knobs (Design 51/59/61 family)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (Design 61, ONE small change: bump is scaled by xs to make it true reactivation)
    ...
    # violated-side boundary bump (same shape as 59/61)
    y = pos / tau
    bump_base = torch.square(y / (1.0 + y)) * torch.exp(-y)  # bump(0)=0, bump'(0)=0, decays ~exp(-y)

    # ONE MODIFICATION vs Design 61: scale bump by xs to suppress unnecessary activation when clause is fully off
    bump = xs * bump_base
