...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- Scaling (MODIFICATION): coupled normalization across v/xl/xs -----
    gmax = torch.maximum(dv.abs().max(), torch.maximum(dxl.abs().max(), dxs.abs().max()))
    scale = 1.0 / gmax.clamp(min=1e-6)
