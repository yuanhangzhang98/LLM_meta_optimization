...
HYPER_SPACE = {
    # Backbone defaults (copied from Design 236)
    ...
    # Hi-ramp sharpness in gate-space (Design 236)
    "hi_sharp": dict(type="log_uniform", default=0.04, low=5e-3, high=0.2),

    # NEW (single principled change vs 236): late-only hinged tail warp on hi
    # For hi <= hi_tail_start: identical to 236.
    # For hi > hi_tail_start: boost remaining slack to 1 with bounded monotone warp.
    "hi_tail_start": dict(type="uniform",     default=0.90, low=0.70, high=0.98),
    "hi_tail_warp":  dict(type="log_uniform", default=2.0,  low=0.8,  high=8.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    sw = s / (s + (1.0 - s).pow(2) + eps)  # fixed rational warp
    ...
    # ===== Hi-ramp breakpoint shift (as in Design 236) =====
    ...
    # ===== Hi-ramp (Design 236) =====
    hi_sharp = torch.as_tensor(hp.hi_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    hi_raw = torch.sigmoid((gate_xl - xl_mid_eff) / hi_sharp)

    sig0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    sig1 = torch.sigmoid((1.0 - xl_mid_eff) / hi_sharp)
    hi = (hi_raw - sig0) / (sig1 - sig0 + 1e-6)
    hi = hi.clamp(0.0, 1.0)

    # ===== SINGLE CHANGE vs Design 236: hinged late-only tail warp on hi =====
    t0 = torch.as_tensor(hp.hi_tail_start, device=v.device, dtype=v.dtype).clamp(0.0, 0.999)
    p = torch.as_tensor(hp.hi_tail_warp, device=v.device, dtype=v.dtype).clamp_min(1e-3)

    denom_tail = (1.0 - t0).clamp_min(1e-6)
    u = ((hi - t0) / denom_tail).clamp(0.0, 1.0)
    u_w = u / (u + (1.0 - u).pow(p) + 1e-12)
    hi_tail = (t0 + denom_tail * u_w).clamp(0.0, 1.0)
    hi = torch.where(hi <= t0, hi, hi_tail)

    xl_gain = torch.as_tensor(hp.xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    release_gain = (1.0 + xl_gain * hi)  # in [1, 1+xl_gain]
