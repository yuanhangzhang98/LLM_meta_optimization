...
HYPER_SPACE = {
    ...
    # Over-gamma shoulder width
    "over_gamma": dict(type="uniform", default=0.03, low=0.0, high=0.15),

    # SINGLE CHANGE vs 386: decouple the upper-edge smoothness from stag_sharp
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Over-gamma shoulder in the upper edge
    over_gamma = torch.as_tensor(hp.over_gamma, device=v.device, dtype=v.dtype).clamp_min(0.0)
    gamma_hi = (gamma + over_gamma).clamp(0.0, 1.0)

    # SINGLE CHANGE vs 386: use a separate sharpness for the upper edge
