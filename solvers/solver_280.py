...
HYPER_SPACE = {
    ...
    # Tighten *when* thr_shift activates: sharpen near_eff with a power > 1.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 228 =====
    # Sharpen the activation used for thr_shift so only deep-in-window clauses shift their xl gate.
    shift_pow = torch.as_tensor(hp.shift_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    near_eff = (near_gamma * weak_band).clamp(0.0, 1.0).pow(shift_pow)
