...
HYPER_SPACE = {
    ...
    # From Design-21/24: faster turn-off vs turn-on
    "eta":     dict(type="uniform",     default=0.2776329339570298,  low=0.0,  high=3.0),

    # From Design-24: satisfied-only minimum off-drive magnitude
    "kappa":   dict(type="uniform",     default=0.02, low=0.0, high=0.25),

    # NEW: smoothness of satisfied-only kappa gate (no leakage for c>gamma because gate depends only on neg)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # but make the kappa floor smoothly satisfied-only (no leakage for c>gamma).
    pos = torch.relu(c - hp.gamma)            # violated beyond gamma
    neg = torch.relu(hp.gamma - c)            # satisfied beyond gamma

    tau = torch.as_tensor(hp.tau_sat, device=c.device, dtype=c.dtype).clamp(min=1e-6)
    # gate in [0,1), exactly 0 when neg=0 (i.e., c>=gamma), approaches 1 for neg>>tau
    gate = 1.0 - torch.exp(-neg / tau)
