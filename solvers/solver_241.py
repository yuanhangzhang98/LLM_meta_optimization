...
HYPER_SPACE = {
    ...
    # SINGLE CHANGE vs Design 240: make upper-edge softening exponent tunable
    # p=0.5 reproduces sqrt(w_hi); smaller p -> less starvation near gamma.
    "w_hi_pow":   dict(type="uniform",     default=0.5,                  low=0.20, high=1.00),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    w_hi_pow = torch.as_tensor(hp.w_hi_pow, device=v.device, dtype=v.dtype).clamp(0.05, 2.0)
    ...
    # ===== Delay-only hi-ramp coupling (as in 238/240; breakpoint independent of weak_band) =====
