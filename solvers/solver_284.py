...
HYPER_SPACE = {
    ...
    # Abs minimum weak-band width (Design 274 knob)
    "band_sharp_abs_min": dict(type="uniform", default=0.01,             low=0.001, high=0.08),

    # NEW (single conceptual change vs 282): allow a small base fraction of the abs-floor
    # even when window≈0, but only when (gate_xl AND push_up) is active.
    # band_floor = abs_min * (1 + gate_xl*push_up*(base + (1-base)*window))
    "band_floor_base":    dict(type="uniform", default=0.15,             low=0.0,   high=0.50),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    push_up = torch.relu(xs_star - xs) / xs_star  # (0,1)
    ...
    window = near_from_below * below_gamma  # (0,1)

    # Weak-sat band: eta < c < gamma
    band_sharp = torch.as_tensor(hp.band_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)

    # --- SINGLE MODIFICATION (vs Design 282 floor strictness) ---
    # Add a small base fraction of the abs-floor outside the window, but only when
    # high-xl clauses still need upward xs motion.
    band_sharp_abs_min = torch.as_tensor(hp.band_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    base = torch.as_tensor(hp.band_floor_base, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    need = gate_xl * push_up
    band_floor = band_sharp_abs_min * (1.0 + need * (base + (1.0 - base) * window))
