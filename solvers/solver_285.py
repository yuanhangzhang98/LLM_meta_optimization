...
HYPER_SPACE = {
    ...
    # Add an explicit dead-zone so thr_shift activates only when near_raw is sufficiently deep-in-window.
    "shift_tau": dict(type="uniform", default=0.12, low=0.0, high=0.80),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 280 =====
    # Dead-zone + renormalized power: selective activation for thr_shift.
    ...
    denom_tau = (1.0 - shift_tau).clamp_min(1e-6)
    u = ((near_raw - shift_tau) / denom_tau).clamp(0.0, 1.0)
    shift_pow = torch.as_tensor(hp.shift_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    near_eff = u.pow(shift_pow)
