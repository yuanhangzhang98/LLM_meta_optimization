...
HYPER_SPACE = {
    ...
    # Below-gamma window geometry (Design 106, one-sided)
    ...
    # In-window amplitude shaping (Design 106)
    ...
    # High-xl release gain
    "xl_gain": dict(type="uniform", default=0.6, low=0.0, high=2.0),

    # Breakpoint in gate-space
    "xl_mid": dict(type="uniform", default=0.5, low=0.2, high=0.8),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== High-xl release gain with tunable breakpoint xl_mid (SINGLE CHANGE vs Design 47):
    # replace piecewise-linear ramp with C^2 smootherstep ramp.
    ...
    t_hi = (torch.relu(gate_xl - xl_mid) / (1.0 - xl_mid).clamp_min(1e-6)).clamp(0.0, 1.0)
    hi = t_hi * t_hi * t_hi * (t_hi * (t_hi * 6.0 - 15.0) + 10.0)  # C^2 in [0,1]

    release_gain = 1.0 + xl_gain * hi  # in [1, 1+xl_gain], ==1 when gate_xl<=xl_mid
