...
HYPER_SPACE = {
    ...
    # NEW: exponent for super-saturation (nu=2 reproduces Design 142)
    # nu>1 ensures p_eff -> 0 as pressure -> inf; also shifts peak to p ~ p_sat/(nu-1).
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Assist: smooth violation gate + exponentiated super-saturation
    ...
    # NEW (single change vs 142): p_eff = p * (p_sat/(p+p_sat))^nu
    # nu=2 recovers 142; larger nu shifts peak left and decays faster at high pressure.
    p_eff = pressure * (p_sat / denom).pow(nu)
