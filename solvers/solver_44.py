...
HYPER_SPACE = {
    ...
    # NEW (single change): bounded, saturating xl-modulation of release magnitude only
    # release *= (1 + rel_xl_gain * gate_xl), with rel_xl_gain kept small
    "rel_xl_gain": dict(type="uniform", default=0.0, low=0.0, high=0.75),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    gate_xl = torch.sigmoid((log_xl - log_thr) / xl_sharp)  # (M,) in (0,1)
    ...
    # SINGLE CHANGE: bounded xl-modulation of release magnitude (no geometry/t-warp dependence)
    rel_xl_gain = torch.as_tensor(hp.rel_xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    release_gain = (1.0 + rel_xl_gain * gate_xl).clamp(1.0, 1.75)

    release = hp.kappa * gate_xl * weak_band * near_gamma * push_up * release_gain
