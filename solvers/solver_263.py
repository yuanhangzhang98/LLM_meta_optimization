...
def _grad_single(vars, idx, sgn, hp):
    ...
    # C∞ compact-support bump tie window (same as 262)
    ...
    # SINGLE CHANGE vs 262: replace softmin(top-2) with bounded Boltzmann expectation in [u1,u2]
    tau = torch.as_tensor(hp.tau_tie, device=v.device, dtype=v.dtype).clamp(min=eps)
    p = torch.softmax(-u12 / tau, dim=-1)   # (M,2)
    c_exp2 = (p * u12).sum(dim=-1)          # (M,) in [u1,u2]

    c_mem = ((1.0 - w) * u1 + w * c_exp2).clamp(0.0, 1.0)
