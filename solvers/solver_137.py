...
HYPER_SPACE = {
    ...
    # Coupled normalization knobs (as in Designs 4/8/10)
    "kappa":     dict(type="log_uniform", default=0.1,   low=1e-3, high=1.0),
    "scale_cap": dict(type="log_uniform", default=100.0, low=1.0,  high=1e4),

    # NEW (single conceptual change vs Design 10): robust magnitude estimator
    # High-quantile of |grad| instead of max(|grad|) to reduce outlier/N-scaling effects.
    "q":         dict(type="uniform",     default=0.95,  low=0.80, high=0.99),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- MODIFICATION: robust-quantile smooth L2 coupled normalization (+cap) ----
    q = torch.as_tensor(hp.q, device=v.device, dtype=v.dtype)

    dv_stat = torch.quantile(dv.abs(), q)
    dxl_stat = torch.quantile(dxl.abs(), q)
    dxs_stat = torch.quantile(dxs.abs(), q)
    mem_stat = torch.maximum(dxl_stat, dxs_stat)

    denom = torch.sqrt(dv_stat * dv_stat + (hp.kappa * mem_stat) * (hp.kappa * mem_stat)).clamp(min=1e-6)
    scale = 1.0 / denom
