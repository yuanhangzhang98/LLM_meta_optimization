...
HYPER_SPACE = {
    ...
    # Tie-only smoothing knobs (as in Design-11/72)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE (vs Design-72): bounded soft-argmin expectation over top-2
    # c_soft in [u1,u2] (no optimism below min)
    tau = hp.tau_tie + 1e-12
    p = torch.softmax(-u12 / tau, dim=-1)            # (M,2)
    c_soft = (p * u12).sum(dim=-1)                   # (M,)
