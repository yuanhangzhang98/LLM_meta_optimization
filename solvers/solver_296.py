...
HYPER_SPACE = {
    ...
    # --- ONE SMALL ADDITION vs baseline ---
    # Anti-starvation floor for effective clause activation in the drive term:
    # xs_eff = xs_floor + (1-xs_floor)*xs  (monotone, bounded, no extra gates)
    "xs_floor": dict(type="uniform",     default=0.05, low=0.0,  high=0.35),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- ONE CHANGE vs baseline: effective xs with amplitude floor (anti-starvation) ---
    xs_floor = torch.as_tensor(hp.xs_floor, device=v.device, dtype=v.dtype).clamp(0.0, 0.99)
    xs_eff = xs_floor + (1.0 - xs_floor) * xs

    G *= (xl * xs_eff).unsqueeze(-1)
