...
HYPER_SPACE = {
    ...
    # NEW (from Design 75): tie-only smoothing parameters for c_mem used ONLY in dxs
    "tau_tie":   dict(type="log_uniform", default=0.017,               low=1e-3, high=2e-1),
    "g0_tie":    dict(type="log_uniform", default=0.0034,              low=1e-3, high=2e-1),
    "kappa_tie": dict(type="log_uniform", default=0.053,               low=1e-3, high=2e-1),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xl stays driven by hard monitor
    dxl = hp.alpha * (c_hard - hp.delta)

    # --- NEW: tie-only smoothed monitor for xs switching (Design 75 style), used ONLY in dxs ---
    # Convert to unsat-per-literal u in [0,1]
    u = 0.5 * (1.0 - v_lit)
    u12, _ = torch.topk(u, 2, dim=-1, largest=False)  # (M,2)
    u1 = u12[:, 0]  # equals c_hard
    gap = (u12[:, 1] - u12[:, 0]).clamp(min=0.0)

    w = torch.sigmoid((hp.g0_tie - gap) / (hp.kappa_tie + 1e-12)).clamp(0.0, 1.0)
    tau = hp.tau_tie
    c_soft2 = (-tau * torch.logsumexp(-u12 / (tau + 1e-12), dim=-1)).clamp(0.0, 1.0)
    c_mem = ((1.0 - w) * u1 + w * c_soft2).clamp(0.0, 1.0)

    # xs update (Design 208), but driven by c_mem instead of c_hard
    pos = torch.relu(c_mem - hp.gamma)   # violated beyond gamma
    neg = torch.relu(hp.gamma - c_mem)   # satisfied beyond gamma

    tau_sat = torch.as_tensor(hp.tau_sat, device=v.device, dtype=v.dtype).clamp(min=1e-12)
    p_sat   = torch.as_tensor(hp.p_sat,   device=v.device, dtype=v.dtype).clamp(min=0.5)

    frac = tau_sat / (neg + tau_sat)
    gate_sat = 1.0 - torch.pow(frac.clamp(0.0, 1.0), p_sat)  # gate_sat(0)=0

    beta_on   = hp.beta
    beta_off  = hp.beta * (1.0 + hp.eta)
    beta_hold = hp.beta * (1.0 + hp.eta_hold)

    # violated-side boundary bump (208), scaled by xs (reactivation-only)
    y = pos / tau_sat
