...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Strong tuned defaults (as in 11/74/113/84)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Base bounded temperature (as in Design 84)
    t_base = 0.25 * torch.minimum(g0, torch.sqrt((g0 * kappa).clamp(min=0.0)))

    # SINGLE MODIFICATION vs Design 84:
    # Adapt temperature using instance-level tie density from a *hard* gap proxy.
    g_hard = torch.maximum(g_mag, g_spread)  # (M,)
    tie_w = torch.sigmoid((g0 - g_hard) / kappa).clamp(0.0, 1.0)
    tie_density = tie_w.mean()  # in [0,1]

    factor = 0.5 + tie_density  # in [0.5, 1.5]
    ...
    w = torch.sigmoid((g0 - gap_eff) / kappa).clamp(0.0, 1.0)
