...
HYPER_SPACE = {
    # Backbone (carry over tuned region used in 325/327/329/333/335/337)
    ...
    # xl-adaptive temperature magnitude + bounded excess (from 337)
    "xl_temp": dict(type="uniform",     default=1.0, low=0.0, high=5.0),
    "xl_cap":  dict(type="log_uniform", default=2.0, low=0.3, high=10.0),

    # SINGLE CHANGE vs 337: apply xl-temp only in a narrow band just below gamma
    # (distance-to-gamma ramp; keeps xl modulation localized, avoids global over-softening)
    "xl_dmax":   dict(type="uniform",     default=0.05, low=0.0,  high=0.25),
    "xl_dsharp": dict(type="log_uniform", default=0.01, low=1e-3, high=0.10),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== xl-adaptive temperature (bounded) =====
    xl_temp = torch.as_tensor(hp.xl_temp, device=v.device, dtype=v.dtype).clamp_min(0.0)
    xl_cap = torch.as_tensor(hp.xl_cap, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    s = (log_xl - log_thr) / xl_sharp
    xl_excess = F.softplus(s)  # >=0
    xl_excess_bounded = torch.tanh(xl_excess / xl_cap)  # in [0,1)

    # ===== SINGLE CHANGE vs 337: distance-to-gamma ramp for xl-temp =====
    # Apply softening only very near gamma (narrow slice of the release window).
    xl_dmax = torch.as_tensor(hp.xl_dmax, device=v.device, dtype=v.dtype).clamp_min(0.0)
    xl_dsharp = torch.as_tensor(hp.xl_dsharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    d = (gamma - c).clamp(0.0, 1.0)  # distance below gamma
    near_gamma_slice = torch.sigmoid((xl_dmax - d) / xl_dsharp)  # ~1 if d < dmax, else ~0

    win_p_eff = win_p / (1.0 + xl_temp * xl_excess_bounded * near_gamma_slice)
    ...
    amp = win_floor + (1.0 - win_floor) * sstep.pow(win_pow)
