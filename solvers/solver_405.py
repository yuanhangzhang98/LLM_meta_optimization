...
HYPER_SPACE = {
    ...
    # Anti-stall micro-release (REDESIGNED trigger: based on rhs_base<0, not xs-low)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs design 403: anti-stall trigger redesign =====
    # Old: depended on xs being low (mis-triggered).
    # New: require xs-decay (rhs_base sufficiently negative) + late/high-xl + top-of-band + tail shut,
    # and cap anti to remain a micro-correction.
    ...
    # decay gate: rhs_base < 0 with margin
    decay_strength = (-rhs_base).clamp_min(0.0)
    decay_gate = torch.sigmoid((decay_strength - anti_rhs_thr) / anti_rhs_sharp).clamp(0.0, 1.0)
