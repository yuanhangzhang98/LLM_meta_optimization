...
HYPER_SPACE = {
    # carried-over tuned defaults (Design 40/47/49-like)
    ...
    # gate family (as in Design 40): zero-leak, finite slope, saturating
    ...
    # NEW (single principled change): independently scale the gated hold-off term
    # beta_off controls turn-off speed via margin neg; beta_hold controls the gated floor strength.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update: asymmetric switch + satisfied-only floor
    ...
    # zero-leak gate (Design 40)
    frac = tau / (neg + tau)
    gate = 1.0 - torch.pow(frac.clamp(0.0, 1.0), p)  # gate(0)=0 exactly

    beta_on   = hp.beta
    beta_off  = hp.beta * (1.0 + hp.eta)             # turn-off speed (margin term)
    beta_hold = hp.beta * (1.0 + hp.eta_hold)        # NEW: independent hold strength (gated floor)

    # NEW (single structural change): split satisfied-side control into two independently-scaled parts
    #   - off via neg (fast turn-off)
    #   - hold via kappa*gate (keep-off without forcing beta_off to also be large)
    dxs = beta_on * (1.0 - xs) * pos \
        - (xs + hp.epsilon) * (beta_off * neg + beta_hold * (hp.kappa * gate))
