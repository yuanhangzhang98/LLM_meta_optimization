...
HYPER_SPACE = {
    ...
    # Super-saturating assist (Design 142 family)
    ...
    # NEW (single conceptual change vs 142): shift the super-sat peak location smoothly
    # k_peak=1 reproduces Design-142 exactly.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: peak-shifted super-saturation (reduces to Design-142 when k_peak=1)
    denom1 = (pressure + p_sat).clamp_min(1e-6)
    denom2 = (pressure + k_peak * p_sat).clamp_min(1e-6)
    p_eff = pressure * (p_sat / denom1) * (p_sat / denom2)
