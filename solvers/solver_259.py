...
HYPER_SPACE = {
    # baseline / 208-family defaults (from 258)
    ...
    # tie-only smoothing parameters for c_mem used ONLY in dxs
    ...
    "kappa_tie": dict(type="log_uniform", default=0.053,               low=1e-4, high=5e-2),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # unsat-per-literal u in [0,1]
    u = 0.5 * (1.0 - v_lit)
    u12, _ = torch.topk(u, 2, dim=-1, largest=False)  # (M,2)
    u1 = u12[:, 0]  # equals c_hard
    gap = (u12[:, 1] - u12[:, 0]).clamp(min=0.0)

    # SINGLE CHANGE vs Design 258: compact-support smootherstep tie window
    # w=1 for gap<=g0, w=0 for gap>=g0+kappa, C2 transition in between.
    eps = 1e-12
    t_raw = (gap - hp.g0_tie) / (hp.kappa_tie + eps)
    t = t_raw.clamp(0.0, 1.0)
    s = t * t * t * (t * (t * 6.0 - 15.0) + 10.0)  # smootherstep in [0,1]
    w_mid = 1.0 - s
    ...
    tau = hp.tau_tie
    c_soft2 = (-tau * torch.logsumexp(-u12 / (tau + eps), dim=-1)).clamp(0.0, 1.0)
