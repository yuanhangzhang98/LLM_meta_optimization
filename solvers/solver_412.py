...
HYPER_SPACE = {
    ...
    # Hinge width (controls how quickly the over-gamma decay turns on)
    "hinge_sharp": dict(type="log_uniform", default=0.01, low=1e-3, high=0.20),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # One-sided squared hinge: overshoot=0 for c<=gamma_hi (no decay leakage),
    # >=0 for c>gamma_hi (so exp() <= 1) => no clamp kink, C1 at boundary.
    ...
    d = c - gamma_hi
    dpos = torch.relu(d)
    overshoot = (dpos * dpos) / (hinge_sharp + 1e-6)
    below_gamma = torch.exp(-overshoot / over_sharp)
