...
HYPER_SPACE = {
    # Backbone (seeded from 363/367 region)
    ...
    # NEW (single change driver): mix floor coupling toward full floor when gate_xl is high
    # 0 => original amp_floor*(c/gamma); 1 => amp_floor when gate_xl=1
    "floor_xl_mix": dict(type="uniform", default=0.35, low=0.0, high=1.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- SINGLE CHANGE vs design 369: xl-gated mixing into the floor coupling ----
    # base = c/gamma (369); when gate_xl is high, tilt multiplier toward 1 to avoid starvation at lower-band c
    floor_xl_mix = torch.as_tensor(hp.floor_xl_mix, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    base = (c / gamma).clamp(0.0, 1.0)
    mult = (base + floor_xl_mix * gate_xl * (1.0 - base)).clamp(0.0, 1.0)
    amp_floor_eff = (amp_floor * mult).clamp(0.0, 0.95)
