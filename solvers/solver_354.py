...
HYPER_SPACE = {
    ...
    # xl-adaptive temperature magnitude (as in 352)
    ...
    # Tail-only gate shaping (from 352)
    "tail_pow": dict(type="uniform", default=3.0, low=1.0, high=6.0),

    # SINGLE CHANGE vs 352: tunable power on instantaneous stall/adapt mask (default recovers sqrt)
    "adapt_pow": dict(type="log_uniform", default=0.5, low=0.25, high=4.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs 352: power-shape the instantaneous stall/adapt mask =====
    adapt_pow = torch.as_tensor(hp.adapt_pow, device=v.device, dtype=v.dtype).clamp_min(0.05)
    adapt_base = (weak_band * window_raw).clamp(0.0, 1.0)
    adapt_mask = adapt_base.pow(adapt_pow) * push_up
