...
def _grad_single(vars, idx, sgn, hp):
    ...
    eps = 1e-6
    # Conservative normalization from the strong cluster (Design-81/83)
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MODIFICATION vs Design-83:
    # cap sqrt-smoothmax smoothing scale by min(kappa, g0) (not kappa alone)
    k = float(hp.kappa)
    g0 = float(hp.g0)
    smooth = max(0.25 * min(k, g0), 1e-4)
    eps_s = smooth * smooth
