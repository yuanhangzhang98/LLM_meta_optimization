...
HYPER_SPACE = {
    ...
    # Near-gamma stagnation proxy (Design 24)
    ...
    # NEW (single change): independent sharpness for the upper edge (below-gamma condition)
    # >1 => softer/broader below-gamma requirement; <1 => sharper/more selective
    "band_hi_mult": dict(type="log_uniform", default=1.0, low=0.2, high=5.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Weak-sat band (single change: upper edge uses its own softness)
    band_hi_mult = torch.as_tensor(hp.band_hi_mult, device=v.device, dtype=v.dtype).clamp_min(1e-3)
    w_lo = torch.sigmoid((c - eta) / band_sharp)
    w_hi = torch.sigmoid((gamma - c) / (band_sharp * band_hi_mult))
    weak_band = w_lo * w_hi
