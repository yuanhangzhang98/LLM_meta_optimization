...
HYPER_SPACE = {
    ...
    # Design-15 style bounded weak-band release (stability fix)
    "kappa":      dict(type="uniform",     default=0.5,   low=0.0,  high=2.0),
    "xl_thr":     dict(type="log_uniform", default=50.0,  low=1.0,  high=1e4),
    "xl_sharp":   dict(type="uniform",     default=1.0,   low=0.1,  high=5.0),
    "eta":        dict(type="uniform",     default=0.02,  low=0.0,  high=0.20),
    "band_sharp": dict(type="log_uniform", default=0.02,  low=1e-3, high=0.20),
    "xs_star":    dict(type="uniform",     default=0.35,  low=0.05, high=0.90),

    # NEW (single principled tweak vs Design 15): smooth the upward-only cutoff
    "xs_tau":     dict(type="log_uniform", default=1e-3,  low=1e-4, high=1e-1),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Baseline xs term + bounded, hysteretic, upward-only release in weak-sat band
    ...
    # Smooth window ~1 for eta < c < gamma, ~0 outside
    ...
    # SINGLE CHANGE vs Design 15:
    # Smooth approximation to relu(xs_star - xs)/xs_star to reduce chatter near xs_star.
    xs_tau = torch.as_tensor(hp.xs_tau, device=v.device, dtype=v.dtype).clamp_min(1e-8)
    push_up = (torch.nn.functional.softplus((xs_star - xs) / xs_tau) * xs_tau / xs_star).clamp(0.0, 1.0)

    release = hp.kappa * gate_xl * weak_band * push_up       # bounded in [0, kappa]
