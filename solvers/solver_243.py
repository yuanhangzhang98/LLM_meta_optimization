...
HYPER_SPACE = {
    ...
    # SINGLE CHANGE vs Design 240: tune the weak_band upper-edge exponent
    # p<1 softens cutoff near gamma (more release availability); p>1 sharpens.
    "w_hi_pow":   dict(type="log_uniform", default=0.5,                  low=0.25, high=2.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs 240: replace sqrt(w_hi) with w_hi**w_hi_pow
    w_hi_pow = torch.as_tensor(hp.w_hi_pow, device=v.device, dtype=v.dtype).clamp_min(1e-3)
    weak_band = w_lo * w_hi.clamp_min(1e-12).pow(w_hi_pow)
    ...
    # ===== Delay-only hi-ramp coupling (no weak_band in breakpoint; avoid double-gating) =====
