...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Start from Design-11 tuned defaults (common best baseline in this run)
    ...
    # Tie-only smoothing knobs (memory monitor only, for dxs)
    "tau_tie": dict(type="log_uniform", default=0.01712294456377018, low=1e-3, high=2e-1),

    # Gate threshold/sharpness for bounded normalized tie-gap (Design-64 style)
    "g0":      dict(type="uniform",     default=0.08, low=1e-3, high=0.5),
    "kappa":   dict(type="log_uniform", default=0.06, low=1e-3, high=0.3),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Memory monitor for dxs: bounded tie gate + entropy-corrected smoother ---
    # u in [0,1]
    u = 0.5 * (1.0 - v_lit)  # (M,3)
    ...
    spread = (u3 - u1).clamp(min=0.0)

    # Design-64 gate: gap_n in [0,0.5], scale-robust
    gap_n = (gap / (gap + spread + 1e-6)).clamp(0.0, 0.5)
    w = torch.sigmoid((hp.g0 - gap_n) / hp.kappa).clamp(0.0, 1.0)

    # Design-71 smoother: entropy-corrected log-mean-exp over 3, clamped to [u1,u2]
    tau = hp.tau_tie
    lme3 = -tau * (torch.logsumexp(-u / tau, dim=-1) - math.log(3.0))
    c_smooth = torch.clamp(lme3, min=u1, max=u2).clamp(0.0, 1.0)

    c_mem = ((1.0 - w) * u1 + w * c_smooth).clamp(0.0, 1.0)

    # Long-term memory stays driven by hard monitor
    dxl = hp.alpha * (c_hard - hp.delta)
    # Short-term switching uses tie-aware monitor
