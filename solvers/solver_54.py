...
HYPER_SPACE = {
    ...
    # NEW: tie-local smoothing width for memory-only clause monitor
    # smaller -> closer to hard min except extremely near ties
    "kappa_c": dict(type="log_uniform", default=3e-2, low=1e-3, high=2e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Memory-only clause monitor: tie-local smoothing of the min ---
    # u in [0,1], hard monitor is min(u). We smooth ONLY near ties between best & 2nd-best literals.
    u = 0.5 * (1.0 - v_lit)  # (M,3)
    ...
    kappa = hp.kappa_c + 1e-12
    w = torch.exp(-gap / kappa)  # ~1 near ties, ~0 when well-separated
    c_mem = (u1 + 0.5 * w * (u2 - u1)).clamp(0.0, 1.0)
