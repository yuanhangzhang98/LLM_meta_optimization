...
VARIABLES_SPEC = {
    ...
    # NEW (single principled change vs 302): clause-local hysteresis latch for tail shutoff
    # xh ~ 1 means "tail locked (release disabled)"; xh ~ 0 means "tail open".
    "xh": dict(init=lambda s: torch.zeros(s),
               shape=("B", "M"),
               bounds=(0.0, 1.0)),
}
...
HYPER_SPACE = {
    # Backbone (seeded from 302 tuned region)
    ...
    # Weak-band xs release (as in 302)
    ...
    # NEW: hysteretic tail latch thresholds (xs_on < xs_off)
    "xs_on":    dict(type="uniform",     default=0.38,  low=0.05, high=0.90),
    "xs_gap":   dict(type="uniform",     default=0.18,  low=0.02, high=0.60),
    "xh_beta":  dict(type="log_uniform", default=30.0,  low=1.0,  high=200.0),
    "xh_sharp": dict(type="log_uniform", default=0.03,  low=1e-3, high=0.20),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # NEW: clause-local hysteresis latch for tail shutoff
    xs_on  = torch.as_tensor(hp.xs_on, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    xs_gap = torch.as_tensor(hp.xs_gap, device=v.device, dtype=v.dtype).clamp_min(0.0)
    xs_off = (xs_on + xs_gap).clamp(0.0, 1.0)

    xh_sharp = torch.as_tensor(hp.xh_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    xh_beta  = torch.as_tensor(hp.xh_beta,  device=v.device, dtype=v.dtype).clamp_min(0.0)

    # lock when xs > xs_off; unlock when xs < xs_on (both smooth)
    lock_w   = torch.sigmoid((xs - xs_off) / xh_sharp)
    unlock_w = torch.sigmoid((xs_on - xs) / xh_sharp)

    # xh in [0,1] with competing flows (no hard branching)
    dxh = xh_beta * (lock_w * (1.0 - xh) - unlock_w * xh)

    # tail gate is simply "not locked"
    gate_tail = (1.0 - xh).clamp(0.0, 1.0)

    kappa = torch.as_tensor(hp.kappa, device=v.device, dtype=v.dtype).clamp_min(0.0)
    release = kappa * gate_xl * weak_band * push_up * gate_tail
