...
HYPER_SPACE = {
    ...
    # Coupled normalization knobs (as in Designs 4/8)
    "kappa":     dict(type="log_uniform", default=0.1,   low=1e-3, high=1.0),
    "scale_cap": dict(type="log_uniform", default=100.0, low=1.0,  high=1e4),

    # NEW (single conceptual change vs Design 8): robust magnitude estimator via p-norm
    # p=inf recovers max; moderate p reduces outlier/N dependence.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- MODIFICATION: p-norm coupled normalization (+cap), replacing max(|.|) ----
    p = torch.as_tensor(hp.p, device=v.device, dtype=v.dtype)

    def pnorm_stat(x):
        # Robust magnitude: (mean |x|^p)^(1/p), strictly positive
