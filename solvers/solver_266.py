...
HYPER_SPACE = {
    # baseline / 208-family defaults (from 258/259; as in 262)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design 262: C² quintic smootherstep tie window
    # w=1 for gap<=g0, w=0 for gap>=g0+kappa, with C² transition in between.
    t_raw = (gap - hp.g0_tie) / (hp.kappa_tie + eps)
    t = t_raw.clamp(0.0, 1.0)
    smoother = t * t * t * (t * (t * 6.0 - 15.0) + 10.0)  # quintic smootherstep
    w_mid = 1.0 - smoother
    w = torch.where(t_raw <= 0.0, torch.ones_like(w_mid), w_mid)
    w = torch.where(t_raw >= 1.0, torch.zeros_like(w), w)
    w = w.clamp(0.0, 1.0)

    # soft-min over top-2 only (as in 262)
