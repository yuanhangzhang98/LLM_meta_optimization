...
HYPER_SPACE = {
    ...
    # ONE-SIDED below-gamma window geometry (as in Design 106)
    ...
    # In-window amplitude shaping (as in Design 106)
    ...
    # NEW (single principled change): bounded xl-dependent gain on release amplitude
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Single change vs Design 106: amplitude gain from xl without shifting c0 =====
    xl_gain = torch.as_tensor(hp.xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    amp_xl = gate_xl * (1.0 + xl_gain * gate_xl)  # still ->0 when xl below thr; boosts high-xl clauses
