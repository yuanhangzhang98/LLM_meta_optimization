...
HYPER_SPACE = {
    # Backbone (kept at tuned defaults from best-known region)
    ...
    # ONE CHANGE vs Design 27: logit-shift warp of w_hi (upper weak-band edge)
    # shift=logit(sqrt(0.5))≈0.88137 reproduces sqrt at w_hi=0.5.
    "w_hi_shift": dict(type="uniform",     default=0.8813735870,        low=0.0,  high=1.5),

    # Below-gamma window geometry (two-sided)
    ...
    # In-window amplitude shaping + skew (as in Design 27)
    ...
    "win_skew":  dict(type="uniform",     default=0.0,               low=-6.0, high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- ONE CHANGE: bounded logit-shift warp of w_hi ---
    ...
    # Below-gamma window: (gamma-stag_margin) < c < gamma (two-sided)
    ...
    # In-window asymmetric ramp (Design 27)
    denom = (gamma - c0).clamp_min(1e-6)
    t = ((c - c0) / denom).clamp(0.0, 1.0)

    skew = torch.as_tensor(hp.win_skew, device=v.device, dtype=v.dtype)
    a = 1.0 + F.softplus(skew)
    b = 1.0 + F.softplus(-skew)
