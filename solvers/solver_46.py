...
HYPER_SPACE = {
    ...
    # Thresholded bounded xl-gain on release amplitude (as in 114)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== ONLY CHANGE vs Design 114: smooth threshold ramp for high-xl gain =====
    # Map gate_xl in [0.5,1] -> t in [0,1], then apply C^2 smootherstep.
    t_hi = ((gate_xl - 0.5) / 0.5).clamp(0.0, 1.0)
    hi = t_hi * t_hi * t_hi * (t_hi * (t_hi * 6.0 - 15.0) + 10.0)  # in [0,1]

    xl_gain = torch.as_tensor(hp.xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    release_gain = (1.0 + xl_gain * hi)  # in [1, 1+xl_gain], and ==1 when gate_xl<=0.5
