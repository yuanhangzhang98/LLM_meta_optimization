...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE MODIFICATION vs Designs-89/109:
    # Regime-consistent temperature for smooth-max of (g_mag, g_spread), tied only to (g0,kappa):
    #   t = 0.25 * min(g0, sqrt(g0*kappa))
    # Ensures t <= g0 (avoid over-soft when g0 small) while allowing t > kappa when kappa << g0.
    g0_f = float(hp.g0)
    k_f = float(hp.kappa)
    t = max(0.25 * min(g0_f, math.sqrt(max(g0_f * k_f, 0.0))), 1e-4)
