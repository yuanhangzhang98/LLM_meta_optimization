...
HYPER_SPACE = {
    ...
    # NEW (vs baseline): clause-local tanh saturator scale for memory updates (after dv-normalization)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: clause-local tanh saturation for memory updates only -----
    # For g<<clip: ~identity; for g>>clip: caps magnitude ~clip.
    g = torch.maximum(dxl.abs(), dxs.abs()).clamp(min=1e-12)   # (M,)
    s = (hp.mem_clip * torch.tanh(g / hp.mem_clip)) / g        # (M,) in (0,1]
    dxl = dxl * s
    dxs = dxs * s
