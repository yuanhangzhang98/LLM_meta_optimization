...
HYPER_SPACE = {
    ...
    # Base fraction for abs-floor (Design 284 knob)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE MODIFICATION vs Design 284 ---
    # Weight the base fraction by near_from_below so the floor persists when window≈0
    # only if the clause is actually approaching the (gamma-margin) edge.
    ...
    base_eff = base * near_from_below
    band_floor = band_sharp_abs_min * (1.0 + need * (base_eff + (1.0 - base_eff) * window))
