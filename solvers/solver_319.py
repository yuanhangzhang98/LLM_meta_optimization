...
HYPER_SPACE = {
    # Backbone defaults (from Design 315 tuned region)
    ...
    # Reused knob: now warps the in-window coordinate t (logit-temperature)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 318: do NOT warp window edges; warp t instead ---
    window_raw = (near_from_below * below_gamma).clamp(0.0, 1.0)

    # In-window amplitude (quintic smootherstep) with logit-temperature warp on t
    ...
    tt = t.clamp(eps, 1.0 - eps)
    logit_t = torch.log(tt) - torch.log1p(-tt)
    t_warp = torch.sigmoid(logit_t * win_p)

    s = t_warp * t_warp * t_warp * (t_warp * (t_warp * 6.0 - 15.0) + 10.0)  # smootherstep
