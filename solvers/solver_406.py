...
HYPER_SPACE = {
    ...
    # Tail-floor amplitude (same parameter; NEW gating in code)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- SINGLE CHANGE vs design 401 ----
    # Tail-floor becomes a calibrated safety valve: engage when tail is shutting down (1-gate_tail_raw)
    # and only when release is weak (1-amp_norm), so it cannot inflate release in healthy regimes.
    tail_floor = torch.as_tensor(hp.tail_floor, device=v.device, dtype=v.dtype).clamp(0.0, 0.95)
    floor_mask = (blend * (1.0 - gate_tail_raw) * (1.0 - amp_norm)).clamp(0.0, 1.0)
