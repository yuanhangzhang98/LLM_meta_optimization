...
HYPER_SPACE = {
    # Backbone (seed from 355 tuned region)
    ...
    # xl-floor for debrittling the floor-mix coupling
    "gate_floor": dict(type="uniform", default=0.05, low=0.0, high=0.5),

    # NEW (single change vs 355): interpolate linear vs quadratic complement
    # rho=1 => 355 behavior; rho=0 => 353 behavior
    "gate_quad": dict(type="uniform", default=0.85, low=0.0, high=1.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # amp_norm: power-law decay + clause-state-driven floor
    ...
    # ---- SINGLE CHANGE vs 355: interpolate linear/quadratic complement in gate_xl_floor ----
    gate_floor = torch.as_tensor(hp.gate_floor, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    gate_quad  = torch.as_tensor(hp.gate_quad,  device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    u = (1.0 - gate_xl).clamp(0.0, 1.0)
    u_mix = ((1.0 - gate_quad) * u + gate_quad * u * u).clamp(0.0, 1.0)
    gate_xl_floor = (gate_xl + gate_floor * u_mix).clamp(0.0, 1.0)
