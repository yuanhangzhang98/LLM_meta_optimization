...
HYPER_SPACE = {
    ...
    # Violation-triggered Top-2 assist (Design 8) + NEW margin gate
    "eta":     dict(type="uniform",     default=0.25, low=0.0,  high=2.0),
    "c_thr":   dict(type="uniform",     default=0.60, low=0.50, high=0.95),
    "m_thr":   dict(type="uniform",     default=0.40, low=0.0,  high=2.0),  # top1-top2 margin threshold
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== NEW: margin-gated violation-triggered assist on runner-up literal =====
    c_thr = torch.as_tensor(hp.c_thr, device=v.device, dtype=v.dtype)
    m_thr = torch.as_tensor(hp.m_thr, device=v.device, dtype=v.dtype)

    # ReLU violation gate (near-zero for mildly violated clauses)
    assist_c = (c - c_thr).clamp_min(0.0)  # (M,)

    # Margin gate: only assist when runner-up is competitive (small top1-top2 margin)
    ...
    assist *= (hp.eta * assist_c * assist_m).unsqueeze(-1) * (xl * xs).unsqueeze(-1)
