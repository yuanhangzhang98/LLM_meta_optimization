...
HYPER_SPACE = {
    ...
    # satisfied-side gate family (Design 40/51)
    ...
    # NEW (single change): violated-side re-activation boost strength
    "kappa_on": dict(type="uniform",     default=0.03,                   low=0.0,  high=0.25),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (Design 51 + ONE change: violated-side boost)
    ...
    # zero-leak, finite-slope, saturating gates
    frac_sat = tau / (neg + tau)
    gate_sat = 1.0 - torch.pow(frac_sat.clamp(0.0, 1.0), p)   # gate_sat(0)=0

    # NEW: same gate family on violated side to steepen re-activation near boundary
    frac_vio = tau / (pos + tau)
    gate_vio = 1.0 - torch.pow(frac_vio.clamp(0.0, 1.0), p)   # gate_vio(0)=0

    beta_on   = hp.beta
    beta_off  = hp.beta * (1.0 + hp.eta)
    beta_hold = hp.beta * (1.0 + hp.eta_hold)

    on_drive  = pos + hp.kappa_on * gate_vio
