...
HYPER_SPACE = {
    ...
    # --- SINGLE CHANGE vs Design 222 ---
    # Near-gamma adaptive xl-threshold shift (in log-space), bounded.
    # log(xl_thr_eff) = log(xl_thr) - thr_shift * near_eff, so xl gate turns on earlier only in-window.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 222 =====
    # Near-gamma adaptive xl-threshold shift for the gate used in release amplitude.
    ...
    log_thr_eff = (log_thr - thr_shift * near_eff).clamp(min=0.0, max=torch.log(torch.as_tensor(1e6, device=v.device, dtype=v.dtype)))
