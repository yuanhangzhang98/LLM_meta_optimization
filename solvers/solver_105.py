...
HYPER_SPACE = {
    ...
    # R routing temperature with near-satisfaction sharpness, but using a smooth sigmoid gate:
    # tau(c) = tau_floor + tau_span * sigmoid((c - c0)/tau_c)
    # => tau ~ tau_floor when c << c0 (sharp only near satisfaction)
    # => tau ~ tau_floor + tau_span when c >> c0 (diffuse when far)
    ...
    "tau_c":     dict(type="log_uniform", default=3e-2, low=1e-3, high=2e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== GRADIENT TERM G (unchanged from Design 11/baseline top-2) =====
    G = c.unsqueeze(-1).repeat(1, 3)
    G.scatter_(1, top_idx[:, 0:1], unsat_top[:, 1:2])
    G *= (xl * xs).unsqueeze(-1)

    # ===== RIGIDITY TERM R (ONLY CHANGE vs Design 11): smooth sigmoid-gated tau(c) =====
    tau_floor = torch.as_tensor(hp.tau_floor, device=v.device, dtype=v.dtype).clamp(min=1e-6)
    tau_span  = torch.as_tensor(hp.tau_span,  device=v.device, dtype=v.dtype).clamp(min=0.0)
    c0        = torch.as_tensor(hp.c0,        device=v.device, dtype=v.dtype)
    tau_c     = torch.as_tensor(hp.tau_c,     device=v.device, dtype=v.dtype).clamp(min=1e-6)

    gate = torch.sigmoid((c - c0) / tau_c)             # (M,) in (0,1)
    tau = (tau_floor + tau_span * gate).clamp(min=1e-6)

    w = torch.softmax(-unsat_top / tau.unsqueeze(-1), dim=-1)  # (M,2)

    R = torch.zeros_like(G)
    R.scatter_(1, top_idx[:, 0:1], (c * w[:, 0]).unsqueeze(-1))
    R.scatter_add_(1, top_idx[:, 1:2], (c * w[:, 1]).unsqueeze(-1))
    ...
    # ===== AUXILIARY VARIABLE GRADIENTS (unchanged) =====
