...
HYPER_SPACE = {
    ...
    # Bounded weak-band release (Design 106/114/46)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    near_gamma = window * amp  # in [0,1]

    # ===== SINGLE CHANGE vs Design 46: near_gamma(+weak_band)-coupled hi-ramp breakpoint =====
    # Symmetric coupling: delay hi-gain when not near (reduces thrash),
    # but advance hi-gain when truly near-gamma in weak-band (faster escape / tail reduction).
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.2, device=v.device, dtype=v.dtype)  # fixed, no new knob

    near_eff = (near_gamma * weak_band).clamp(0.0, 1.0)
