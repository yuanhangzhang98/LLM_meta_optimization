...
HYPER_SPACE = {
    ...
    # NEW (single-change): weak_band-only floor activation (soft threshold)
    "wb_floor_thr":   dict(type="uniform",     default=0.18,  low=0.0,  high=0.9),
    "wb_floor_sharp": dict(type="log_uniform", default=0.05,  low=1e-3, high=0.2),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 340: floor activates only when weak_band is genuinely “on” ---
    wb_thr   = torch.as_tensor(hp.wb_floor_thr,   device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    wb_sharp = torch.as_tensor(hp.wb_floor_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    floor_gate = torch.sigmoid((weak_band - wb_thr) / wb_sharp).clamp(0.0, 1.0)
