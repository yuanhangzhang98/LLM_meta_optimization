...
HYPER_SPACE = {
    ...
    # NEW (single change vs Design 235): ultra-late smooth tail-only boost of hi toward 1
    # Guarantees: if hi <= hi_tail_start, dynamics identical to Design 235.
    "hi_tail_start": dict(type="uniform", default=0.97, low=0.90, high=0.995),
    # Bounded in [0,1]: boosts only remaining slack (1-hi) so effective hi stays in [hi,1]
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Hi-ramp shift logic from Design 235 (unchanged) =====
    mid0 = torch.as_tensor(0.5, device=v.device, dtype=v.dtype)
    mid_shift = torch.as_tensor(0.2, device=v.device, dtype=v.dtype)  # fixed
    ...
    # ===== SINGLE CHANGE vs Design 235: ultra-late tail-only boost of hi (smooth, bounded) =====
    ...
    # eff_hi == hi for hi<=t0; otherwise nudges toward 1 by boosting remaining slack (1-hi)
    eff_hi = (hi + tb * tail * (1.0 - hi)).clamp(0.0, 1.0)
