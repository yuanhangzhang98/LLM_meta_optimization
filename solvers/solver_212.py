...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Strong tuned defaults (as in Design-84)
    ...
    # SINGLE ADDITION vs Design-84: clause-severity cutoff for enabling smoothing
    # Smaller u_sev => smoothing only when clause is already near-satisfied.
    "u_sev":   dict(type="uniform", default=0.35, low=0.05, high=0.50),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    g0 = v.new_tensor(float(hp.g0))
    kappa = v.new_tensor(float(hp.kappa))
    t_base = 0.25 * torch.minimum(g0, torch.sqrt((g0 * kappa).clamp(min=0.0)))

    u_bar = u1.mean()
    s = u_bar / (u_bar + 0.25)
    factor = 0.5 + s
    t = (t_base * factor).clamp(min=1e-4)

    pair = torch.stack([g_mag / t, g_spread / t], dim=-1)
    gap_eff = (t * torch.logsumexp(pair, dim=-1) - t * math.log(2.0)).clamp(0.0, 1.0)

    w_tie = torch.sigmoid((hp.g0 - gap_eff) / (hp.kappa + 1e-12)).clamp(0.0, 1.0)

    # SINGLE CHANGE vs Design-84: severity-gate smoothing off for clearly violated clauses
    sev = torch.sigmoid((hp.u_sev - u1) / (hp.kappa + 1e-12)).clamp(0.0, 1.0)
    w = (w_tie * sev).clamp(0.0, 1.0)

    tau = hp.tau_tie
    c_soft = (-tau * torch.logsumexp(-u / (tau + 1e-12), dim=-1)).clamp(0.0, 1.0)
