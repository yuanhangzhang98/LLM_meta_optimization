...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Strong tuned defaults (as in 81/85)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Conservative normalization (as in Design 85)
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MODIFICATION vs Design 85:
    # Couple softmax-aggregation temperature to BOTH g0 and kappa.
    # This makes the softmax-weighted average less biased downward when g0 is small,
    # keeping the tie-gate off unless the gap is genuinely tiny.
    g0_f = float(hp.g0)
    k_f = float(hp.kappa)
    t = max(0.25 * math.sqrt(max(g0_f * k_f, 0.0)), 1e-4)
