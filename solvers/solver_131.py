...
HYPER_SPACE = {
    ...
    # NEW: relative time-scale for memory vs v (paired with decoupled normalization)
    "mem_rel": dict(type="log_uniform", default=1.0,   low=0.1,  high=10.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- MODIFICATION: decoupled normalization (v vs memory) ----
    scale_v = 1.0 / (dv.abs().max().clamp(min=1e-6))
    mem_max = torch.maximum(dxl.abs().max(), dxs.abs().max())
    scale_m = 1.0 / (mem_max.clamp(min=1e-6))

    dv  = -dv  * scale_v
    dxl = -dxl * scale_m * hp.mem_rel
    dxs = -dxs * scale_m * hp.mem_rel
