...
# ------------------------------------------------------------------ #
# 2.  HYPER_SPACE (Design-76 defaults)
# ------------------------------------------------------------------ #
HYPER_SPACE = {
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Softmax-combine two normalizations (as in 76)
    t = max(0.25 * float(hp.kappa) * (1.0 + 0.5 * float(hp.g0)), 1e-4)
    pair = torch.stack([g_mag / t, g_spread / t], dim=-1)
    gap_eff = (t * torch.logsumexp(pair, dim=-1) - t * math.log(2.0)).clamp(0.0, 1.0)

    # SINGLE CHANGE vs Design-76:
    # concave warp makes the tie gate activate only for *very* small gaps
    gap_eff = torch.sqrt(gap_eff + 1e-12).clamp(0.0, 1.0)
