...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Strong tuned defaults (as in 11/74/113)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE MODIFICATION vs Design-113:
    # Make the smooth-max temperature regime-adaptive using instance mean(u1).
    # Keep bounded: t_base <= 0.25*g0, factor in [0.5, 1.5].
    ...
    u_bar = u1.mean()  # instance-level regime statistic (early high, late low)
    s = u_bar / (u_bar + 0.25)  # smooth in [0,1]
