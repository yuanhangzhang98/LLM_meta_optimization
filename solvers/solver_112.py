...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ---- SINGLE MODIFICATION vs baseline: coupled RMS normalization ----
    # Replace outlier-dominated max(|dv|) scaling with a robust shared RMS scale
    # computed from (dv, dxl, dxs) to keep v/memory time-scales coupled as size grows.
    rms_v = dv.pow(2).mean().sqrt()
    rms_l = dxl.pow(2).mean().sqrt()
    rms_s = dxs.pow(2).mean().sqrt()
    rms_mem = torch.sqrt(0.5 * (rms_l * rms_l + rms_s * rms_s))
    denom = torch.sqrt(rms_v * rms_v + rms_mem * rms_mem).clamp(min=1e-6)
    scale = 1.0 / denom
