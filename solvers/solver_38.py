...
HYPER_SPACE = {
    ...
    # In-window amplitude shaping (same params as 83)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ONE change vs Design 83: apply win_pow BEFORE the fixed rational warp
    # (smootherstep -> power -> warp) instead of (smootherstep -> warp -> power).
    ...
    sp = s.pow(win_pow)  # moved earlier

    eps = torch.as_tensor(1e-12, device=v.device, dtype=v.dtype)
    sw = sp / (sp + (1.0 - sp).pow(2) + eps)  # warp after power
