...
HYPER_SPACE = {
    ...
    # Hi-ramp sharpness (kept identical to Design 236; only mapping changes)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== SINGLE CHANGE vs Design 236: normalized tanh hi-ramp in gate-space =====
    hi_sharp = torch.as_tensor(hp.hi_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    denom_hi = (4.0 * hi_sharp).clamp_min(1e-6)  # match local slope scale of sigmoid at midpoint

    u = torch.tanh((gate_xl - xl_mid_eff) / denom_hi)  # in (-1,1), u=0 at gate_xl=xl_mid_eff
    u1 = torch.tanh((1.0 - xl_mid_eff) / denom_hi)     # >0
    hi = (u / (u1 + 1e-6)).clamp(0.0, 1.0)
