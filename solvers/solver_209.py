...
# 2.  HYPER_SPACE
# ------------------------------------------------------------------ #
# Keep the strong tuned defaults used by recent best designs
HYPER_SPACE = {
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Same features as Design-85
    g_mag = (gap / u2.clamp(min=eps)).clamp(0.0, 1.0)
    g_spread = (gap / (u3 - u1).clamp(min=eps)).clamp(0.0, 1.0)

    # SINGLE MODIFICATION vs Design-85:
    # Make the softmax-avg temperature regime-adaptive using instance mean(u1).
    # Early (u1 large): larger t => less brittle tie decisions.
    # Late  (u1 small): smaller t => recover near-hard max behavior.
    kappa = v.new_tensor(float(hp.kappa))
    t_base = 0.25 * kappa
    u_bar = u1.mean()                      # scalar tensor
    s = u_bar / (u_bar + 0.25)             # in [0,1]
    factor = 0.5 + s                       # in [0.5, 1.5]
    t = (t_base * factor).clamp(min=1e-4)  # scalar

    pair = torch.stack([g_mag, g_spread], dim=-1)          # (M,2)
    wts = torch.softmax(pair / t, dim=-1)                  # (M,2)
    gap_eff = (wts * pair).sum(dim=-1).clamp(0.0, 1.0)     # (M,)
