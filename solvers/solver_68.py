...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    # Start from Design-11 tuned defaults
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # u12 are the two smallest costs (u1<=u2)
    ...
    # softmin computed over only the two contenders (u1,u2), not all three literals
    tau = hp.tau_tie
    c_soft_top2 = (-tau * torch.logsumexp(-u12 / tau, dim=-1)).clamp(0.0, 1.0)  # (M,)
    c_mem = ((1.0 - w) * u1 + w * c_soft_top2).clamp(0.0, 1.0)

    # Long-term memory stays driven by hard monitor
    dxl = hp.alpha * (c_hard - hp.delta)
    # Short-term switching uses refined tie-only smoothing
