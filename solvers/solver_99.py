...
HYPER_SPACE = {
    ...
    # c-adaptive rigidity routing temperature (sharp when c is small, smooth when c is large)
    "tau_floor": dict(type="log_uniform", default=1e-3, low=1e-6, high=5e-2),
    "tau_span":  dict(type="log_uniform", default=2e-1, low=1e-4, high=7e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== RIGIDITY TERM R (ONLY CHANGE): top-2 soft routing with c-dependent sharpness =====
    tau = (hp.tau_floor + hp.tau_span * c).clamp(min=1e-6)            # (M,)
