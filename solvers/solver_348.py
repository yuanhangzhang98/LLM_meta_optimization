...
VARIABLES_SPEC = {
    ...
    # NEW: per-clause persistence / stall accumulator (0..1)
    "xt": dict(init=lambda s: torch.zeros(s),
               shape=("B", "M"),
               bounds=(0.0, 1.0)),
}
...
HYPER_SPACE = {
    # Backbone (carry over tuned region used in 325/327/329)
    ...
    # NEW: persistence time-constant for stall accumulator
    "tau": dict(type="log_uniform", default=5.0, low=0.5, high=50.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    gate_tail = torch.sigmoid((log_xl - log_thr_tail) / xl_sharp)

    # Instantaneous stall signal (same as 346’s broadened adapt mask)
    stall_sig = torch.sqrt((weak_band * window_raw).clamp(0.0, 1.0)) * push_up

    # NEW: persistence-based gate (EMA): xt tracks how long we stay in stall regime
    tau = torch.as_tensor(hp.tau, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    dxt = tau * (stall_sig - xt)

    # Temperature only after persistence (targets tail stalls)
    win_p_eff = win_p / (1.0 + xl_temp * gate_tail * xt)
    ...
    return {"v": dv, "xl": dxl, "xs": dxs, "xt": dxt}
