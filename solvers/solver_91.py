...
HYPER_SPACE = {
    ...
    # NEW: cap memory (xl/xs) update magnitude after dv-based normalization
    # (dv is normalized to max |dv| ~= 1; this bounds max |dx| to ~mem_clip)
    "mem_clip": dict(type="log_uniform", default=5.0,  low=0.1,  high=20.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ----- NEW: smooth global clip for memory updates only -----
    mem_norm = torch.maximum(dxl.abs().max(), dxs.abs().max()).clamp(min=1e-12)
    mem_damp = 1.0 / (1.0 + (mem_norm / hp.mem_clip))  # in (0,1]
    dxl = dxl * mem_damp
    dxs = dxs * mem_damp
