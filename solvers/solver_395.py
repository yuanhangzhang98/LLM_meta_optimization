...
HYPER_SPACE = {
    ...
    # Over-gamma shoulder (relative width), PLUS absolute cap
    "over_gamma": dict(type="uniform", default=0.03,  low=0.0, high=0.15),
    "over_cap":   dict(type="uniform", default=0.02,  low=0.0, high=0.08),

    # SINGLE CHANGE vs 392: bounded coupling for upper-edge sharpness
    # over_sharp := stag_sharp * over_sharp_ratio (keeps edges coupled; ratio=1 recovers 392)
    "over_sharp_ratio": dict(type="uniform", default=1.0, low=0.5, high=2.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Below-gamma window: (gamma-stag_margin) < c < gamma_hi (two-sided)
    ...
    # --- SINGLE CHANGE vs 392 ---
    over_sharp_ratio = torch.as_tensor(hp.over_sharp_ratio, device=v.device, dtype=v.dtype)
    over_sharp = (stag_sharp * over_sharp_ratio).clamp(1e-6, 0.20)
    below_gamma = torch.sigmoid((gamma_hi - c) / over_sharp)
