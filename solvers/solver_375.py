...
HYPER_SPACE = {
    ...
    # Convex/late control (>=1), applied after smoothstep
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs design 373: smoothstep-shaped in-window floor coupling (slower early, bounded)
    s = (t * t * (3.0 - 2.0 * t)).clamp(0.0, 1.0)  # smoothstep
    floor_pow = torch.as_tensor(hp.floor_pow, device=v.device, dtype=v.dtype).clamp_min(1.0)
    amp_floor_eff = (amp_floor * torch.pow(s, floor_pow)).clamp(0.0, 0.95)
