...
HYPER_SPACE = {
    ...
    # NEW: self-hysteresis strength for xs turn-off threshold.
    # hys=0 recovers Design-10 asymmetric xs switch.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update (ONLY CHANGE): asymmetric switch + self-hysteresis on turn-off.
    # Turn-on threshold stays at gamma.
    pos = torch.relu(c - hp.gamma)

    # Turn-off threshold decreases with xs (if already on, require stronger satisfaction to turn off).
    gamma_off = torch.clamp(hp.gamma - hp.hys * xs, 0.0, 0.5)
