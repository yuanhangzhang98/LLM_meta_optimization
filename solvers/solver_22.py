...
HYPER_SPACE = {
    ...
    # Bounded weak-band release (Design-49 backbone)
    ...
    # In-window amplitude shaping (Design 49)
    "win_floor": dict(type="uniform",     default=0.35, low=0.0,  high=0.90),
    "win_pow":   dict(type="log_uniform", default=1.0,  low=0.25, high=4.0),

    # NEW (single change): xl-conditioned upper-edge softness for weak-band
    # hi_mult_eff = 1 + band_hi_xl * gate_xl, so:
    # - low xl (gate_xl~0): sharp/selective upper edge
    # - high xl (gate_xl~1): softer upper edge (targeted release for hard clauses)
    "band_hi_xl": dict(type="uniform", default=0.0, low=0.0, high=6.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Design 49: upper edge softness depends on xl
    band_hi_xl = torch.as_tensor(hp.band_hi_xl, device=v.device, dtype=v.dtype).clamp_min(0.0)
    hi_mult_eff = 1.0 + band_hi_xl * gate_xl  # in [1, 1+band_hi_xl]
    w_hi = torch.sigmoid((gamma - c) / (band_sharp * hi_mult_eff).clamp_min(1e-6))
    ...
    amp = win_floor + (1.0 - win_floor) * t.pow(win_pow)
