...
HYPER_SPACE = {
    ...
    # NEW: tie-only smoothing for memory monitor used in dxs
    # tau_tie: softmin temperature when we decide we are in a tie regime
    "tau_tie": dict(type="log_uniform", default=0.01712294456377018, low=1e-3, high=2e-1),
    # g0: gap threshold where mixing turns on (gap = u2 - u1)
    "g0":      dict(type="log_uniform", default=0.0033876327188782085, low=1e-3, high=2e-1),
    # kappa: sharpness of the on/off transition (smaller => more hard-min almost everywhere)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # w ~ 1 when gap << g0 (tie), w ~ 0 when gap >> g0 (clear winner)
    ...
    # Long-term memory stays driven by hard monitor to preserve targeting
    dxl = hp.alpha * (c_hard - hp.delta)
    # Short-term switching uses tie-only smoothing to reduce literal-switch chattering
