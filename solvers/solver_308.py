...
HYPER_SPACE = {
    # baseline / 208-family defaults (from 262/268)
    ...
    # NEW: gap-adaptive near-min correction (used ONLY in dxs via c_mem)
    # tau_tie sets the gap scale where tie-smoothing is active.
    # a_tie caps how far above u1 the monitor can move (preserves winner identity).
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Gap-adaptive, bounded near-min correction:
    # c_mem in [u1, u2], c_mem -> u1 as gap >> tau_tie
    ...
    tau2 = tau * tau
    phi = a * (tau2 / (gap * gap + tau2 + eps))  # in [0,a]
    c_mem = (u1 + gap * phi).clamp(0.0, 1.0)
