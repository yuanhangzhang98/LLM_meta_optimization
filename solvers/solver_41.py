...
HYPER_SPACE = {
    ...
    # Below-gamma window geometry (one-sided) + ONE CHANGE: xl-adaptive margin
    "stag_margin": dict(type="uniform",     default=0.2061293027725612,  low=0.0,  high=0.25),
    "stag_gain":   dict(type="uniform",     default=0.06,                low=0.0,  high=0.25),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== Below-gamma window (ONE-SIDED) with xl-adaptive lower edge =====
    stag_margin = torch.as_tensor(hp.stag_margin, device=v.device, dtype=v.dtype).clamp_min(0.0)
    stag_gain = torch.as_tensor(hp.stag_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    stag_sharp = torch.as_tensor(hp.stag_sharp, device=v.device, dtype=v.dtype).clamp_min(1e-6)

    # Effective margin widens only for high-xl clauses (gate_xl ~ 1)
    margin_eff = stag_margin + stag_gain * gate_xl
    margin_eff = torch.minimum(margin_eff, (gamma - 1e-3).clamp_min(1e-6))

    c0 = (gamma - margin_eff).clamp(0.0, 1.0)
    window = torch.sigmoid((c - c0) / stag_sharp)  # one-sided; weak_band shuts off above gamma
