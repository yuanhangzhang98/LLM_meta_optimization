...
HYPER_SPACE = {
    ...
    # NEW: persistence amplification exponent for xl update (kappa=0 recovers baseline)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ONLY CHANGE vs baseline: persistence-amplified xl update
    # dxl = alpha * (c - delta) * xl^kappa
    mult = torch.pow(xl, hp.kappa)
    dxl = hp.alpha * (c - hp.delta) * mult
