...
HYPER_SPACE = {
    # Baseline (kept at best-known defaults from Design 51/67 lineage)
    ...
    # Bounded weak-band release (unchanged)
    ...
    # Below-gamma window geometry (unchanged)
    ...
    # In-window amplitude shaping (same knobs)
    ...
    # ONE new knob: gentle, bounded skew of the in-window ramp (0 => symmetric)
    "ramp_skew": dict(type="uniform",     default=0.0,                   low=-0.8, high=0.8),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # In-window amplitude: ONE small principled change vs Design 67
    # Use a smooth power-ratio warp (edge-flat) before quintic smootherstep.
    denom = (gamma - c0).clamp_min(1e-6)
    t = ((c - c0) / denom).clamp(0.0, 1.0)

    # warp: t -> t_tilde = t^p / (t^p + (1-t)^q), with p fixed=2 and q controlled by ramp_skew
    ramp_skew = torch.as_tensor(hp.ramp_skew, device=v.device, dtype=v.dtype).clamp(-1.0, 1.0)
    p = torch.as_tensor(2.0, device=v.device, dtype=v.dtype)
    q = (2.0 * torch.exp(ramp_skew)).clamp_min(1.05)  # keep edge derivative ~0 at t=1

    tp = t.pow(p)
    oq = (1.0 - t).pow(q)
    t_tilde = tp / (tp + oq + 1e-12)

    # quintic smootherstep on warped coordinate (C^2, flat edges)
    s = t_tilde * t_tilde * t_tilde * (t_tilde * (t_tilde * 6.0 - 15.0) + 10.0)
