...
HYPER_SPACE = {
    ...
    # Tie-only smoothing knobs (compact-support gate)
    "tau_tie": dict(type="log_uniform", default=0.01712294456377018, low=1e-3, high=2e-1),
    "g0":      dict(type="log_uniform", default=0.01,               low=1e-3, high=2e-1),
    "kappa":   dict(type="log_uniform", default=0.01,               low=1e-3, high=1e-1),
}
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # SINGLE CHANGE vs Designs 54/58:
    # Reparameterize compact-support window so w=1 for gap<=g0, and w=0 for gap>=g0+kappa.
    # This guarantees full tie-activation at exact ties even when g0 < kappa.
    t = ((gap - hp.g0) / hp.kappa).clamp(0.0, 1.0)
    # Quintic smoothstep on t, then flip so it decays from 1 to 0
    s = (t * t * t * (10.0 - 15.0 * t + 6.0 * t * t)).clamp(0.0, 1.0)
    w = (1.0 - s).clamp(0.0, 1.0)
