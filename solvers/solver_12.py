...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- Single change vs Design 24: make near-gamma strictly a below-gamma window ---
    ...
    # Windowed detector: ~1 only when (gamma-stag_margin) < c < gamma
    near_from_below = torch.sigmoid((c - c0) / stag_sharp)
    below_gamma = torch.sigmoid((gamma - c) / stag_sharp)
    near_gamma = near_from_below * below_gamma
