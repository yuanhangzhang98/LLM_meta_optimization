...
HYPER_SPACE = {
    ...
    # Same parameter as 245, but we will *gate* its effect by gate_xl (no new hparams)
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 245: gate the logit-shift by gate_xl to delay activation ---
    w_hi_shift = torch.as_tensor(hp.w_hi_shift, device=v.device, dtype=v.dtype).clamp(0.0, 5.0)
    shift_eff = (w_hi_shift * gate_xl).clamp(0.0, 5.0)
    w_hi_c = w_hi.clamp(1e-6, 1.0 - 1e-6)
    w_hi_soft = torch.sigmoid(torch.logit(w_hi_c) + shift_eff)
