...
HYPER_SPACE = {
    # carried-over tuned defaults (Design 40/47-like)
    ...
    # NEW gate knobs (same knobs as Exp-47, but new gate shape)
    "tau_sat": dict(type="log_uniform", default=0.01,                  low=1e-5, high=2e-1),
    "s_sat":   dict(type="log_uniform", default=1.0,                   low=0.2,  high=50.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # xs update: asymmetric switch + satisfied-only kappa floor with NEW hard-saturating zero-leak gate
    ...
    # NEW (single change vs Exp-47): hard-saturating ramp gate
    # gate = clamp(s * neg / tau, 0, 1)
    # Properties: gate(0)=0 exactly (zero-leak); gate'(0)=s/tau (finite, tunable);
    #             saturates exactly to 1 to eliminate long-tail off-drive jitter.
    gate = ((s * neg) / tau).clamp(0.0, 1.0)
