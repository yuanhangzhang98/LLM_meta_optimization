...
HYPER_SPACE = {
    ...
    # Bounded weak-band release (Design 106)
    ...
    # ONE small change: thresholded, bounded xl-conditioned gain on release amplitude
    # Gain is 1 when gate_xl<=0.5 (xl below threshold region), rises to (1+xl_gain) as gate_xl->1.
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # ===== ONE CHANGE vs Design 106: bounded, thresholded xl-gain on release amplitude =====
    # gate_xl=0.5 corresponds to xl=xl_thr. No extra boost below threshold.
    xl_gain = torch.as_tensor(hp.xl_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    hi = torch.relu(2.0 * gate_xl - 1.0)  # in [0,1]
