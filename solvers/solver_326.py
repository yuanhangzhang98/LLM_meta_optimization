...
HYPER_SPACE = {
    # Backbone (seeded from 321 tuned region)
    ...
    # Self-limiting amplitude at huge xl (SINGLE CHANGE vs 321: new amp_norm family)
    "xl_norm":   dict(type="log_uniform", default=15292.311320487264, low=1e3,  high=1e6),
    "amp_sigma": dict(type="log_uniform", default=1.0,             low=0.10, high=5.0),
    "amp_p":     dict(type="uniform",     default=2.0,             low=1.0,  high=4.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # tail-safety gate (anti-starvation sign as in 316/321)
    ...
    # SINGLE CHANGE vs 321: log-space stretched-exponential amp_norm (monotone, bounded, stronger tail)
    ...
    t = torch.relu(log_xl - log_norm)  # 0 for xl<=xl_norm, grows for xl>xl_norm
    amp_norm = torch.exp(-torch.pow(t / amp_sigma, amp_p)).clamp(0.0, 1.0)
