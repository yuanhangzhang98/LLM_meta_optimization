...
HYPER_SPACE = {
    ...
    "k_peak":  dict(type="log_uniform", default=1.0,    low=0.25,  high=4.0),

    # NEW (single modification vs 174): tail-safe floor for super-saturation factor
    # rho=0 reproduces 174 exactly; rho>0 prevents p_eff from vanishing at high pressure.
    "rho":     dict(type="uniform",     default=0.2,    low=0.0,   high=0.9),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # Assist: smooth violation gate + super-saturating effective pressure
    ...
    k_peak = torch.as_tensor(hp.k_peak, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    rho    = torch.as_tensor(hp.rho,    device=v.device, dtype=v.dtype).clamp(0.0, 1.0)

    z = ((c - c_thr) / w).clamp(-20.0, 20.0)
    gate = torch.sigmoid(z)  # (M,)

    # Design-174 p_eff with a tail-safe floor (single change):
    # base_sat = pressure * p_sat/(pressure+p_sat)
    # super_factor = p_sat/(pressure+k_peak*p_sat)
    # new: super_factor := rho + (1-rho)*super_factor  => p_eff -> rho*p_sat as pressure->inf
    denom1 = (pressure + p_sat).clamp_min(1e-6)
    base_sat = pressure * (p_sat / denom1)

    denom2 = (pressure + k_peak * p_sat).clamp_min(1e-6)
    super_factor = (p_sat / denom2)
    super_factor = rho + (1.0 - rho) * super_factor

    p_eff = base_sat * super_factor
    ...
    # Memory updates (baseline; no leak/forgetting)
