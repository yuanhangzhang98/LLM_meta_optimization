...
def _grad_single(vars, idx, sgn, hp):
    ...
    # CHANGE vs 329:
    # use a non-saturating, smooth log-xl excess (softplus) for temperature adaptation,
    # instead of gate_xl which saturates at 1 for very large xl.
    xl_temp = torch.as_tensor(hp.xl_temp, device=v.device, dtype=v.dtype).clamp_min(0.0)
    s = (log_xl - log_thr) / xl_sharp
    xl_excess = F.softplus(s)  # ~0 below thr, grows ~linearly in log-xl above thr
    win_p_eff = win_p / (1.0 + xl_temp * xl_excess)
    ...
    sstep = t * t * t * (t * (t * 6.0 - 15.0) + 10.0)  # smootherstep
