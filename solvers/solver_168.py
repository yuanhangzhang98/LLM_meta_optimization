...
# ------------------------------------------------------------------ #
HYPER_SPACE = {
    "alpha":   dict(type="log_uniform", default=6.772139667214615,   low=0.5,  high=50.0),
    "beta":    dict(type="log_uniform", default=19.732017223438238,  low=2.0,  high=200.0),
    "gamma":   dict(type="uniform",     default=0.23908624550473975,   low=0.01, high=0.50),
    "delta":   dict(type="uniform",     default=0.038602565187111644, low=0.01, high=0.50),
    "epsilon": dict(type="log_uniform", default=0.0002798981292900607, low=1e-4, high=1e-2),
    "zeta":    dict(type="log_uniform", default=0.003989214800623618, low=1e-4, high=1.0),

    # Top-2 assist (Design 61): bounded amplitude via pressure saturation
    "eta":     dict(type="uniform",     default=1.6275511175587607,   low=0.0,  high=2.0),
    "c_thr":   dict(type="uniform",     default=0.5232873397223186,   low=0.50, high=0.95),
    "p_sat":   dict(type="log_uniform", default=493.98977757272917,   low=0.3,  high=1e3),

    # NEW (single change vs Design 61): dimensionless ambiguity/margin gate
    # r = (top1-top2)/(1-top1+eps); assist only if r < r_thr
    "r_thr":   dict(type="uniform",     default=0.3927174266037837,                low=0.0,  high=3.0),

    "lr":      dict(type="log_uniform", default=0.7621290371875519,   low=1e-1, high=3.0),
}
