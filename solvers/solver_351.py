...
HYPER_SPACE = {
    ...
    # Soft-OR mix weight (kept)
    "floor_mix": dict(type="uniform", default=1.0, low=0.0, high=1.0),

    # NEW (single change vs 347): leaky xl->floor_mix coupling (0 recovers 347; small >0 improves robustness)
    "mix_leak": dict(type="uniform", default=0.08, low=0.0, high=0.5),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs 347: leaky xl-adaptive floor-mix (no lag state) ---
    floor_mix = torch.as_tensor(hp.floor_mix, device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    mix_leak  = torch.as_tensor(hp.mix_leak,  device=v.device, dtype=v.dtype).clamp(0.0, 1.0)
    floor_mix_eff = (floor_mix * (mix_leak + (1.0 - mix_leak) * gate_xl)).clamp(0.0, 1.0)
