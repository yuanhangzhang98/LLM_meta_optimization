...
HYPER_SPACE = {
    ...
    # NEW (single change vs 274): tunable one-sided xl-widening gain for the abs-width floor
    # band_floor = abs_min * (1 + band_floor_gain * gate_xl)
    "band_floor_gain":    dict(type="uniform", default=0.7,              low=0.0,   high=2.0),
...
def _grad_single(vars, idx, sgn, hp):
    ...
    # --- SINGLE CHANGE vs Design 274: tunable one-sided widening gain for abs floor ---
    band_sharp_abs_min = torch.as_tensor(hp.band_sharp_abs_min, device=v.device, dtype=v.dtype).clamp_min(1e-6)
    band_floor_gain = torch.as_tensor(hp.band_floor_gain, device=v.device, dtype=v.dtype).clamp_min(0.0)
    band_floor = band_sharp_abs_min * (1.0 + band_floor_gain * gate_xl)
